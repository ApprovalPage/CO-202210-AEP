(function (cjs, an) {

var p; // shortcut to reference prototypes
var lib={};var ss={};var img={};
lib.ssMetadata = [
		{name:"CO_AEP_728x90_atlas_NP_1", frames: [[0,0,600,264],[0,266,600,264],[0,532,600,264],[0,798,600,264]]}
];


(lib.AnMovieClip = function(){
	this.actionFrames = [];
	this.ignorePause = false;
	this.gotoAndPlay = function(positionOrLabel){
		cjs.MovieClip.prototype.gotoAndPlay.call(this,positionOrLabel);
	}
	this.play = function(){
		cjs.MovieClip.prototype.play.call(this);
	}
	this.gotoAndStop = function(positionOrLabel){
		cjs.MovieClip.prototype.gotoAndStop.call(this,positionOrLabel);
	}
	this.stop = function(){
		cjs.MovieClip.prototype.stop.call(this);
	}
}).prototype = p = new cjs.MovieClip();
// symbols:



(lib.CO_AEP_300x2501 = function() {
	this.initialize(ss["CO_AEP_728x90_atlas_NP_1"]);
	this.gotoAndStop(0);
}).prototype = p = new cjs.Sprite();



(lib.CO_AEP_300x2502 = function() {
	this.initialize(ss["CO_AEP_728x90_atlas_NP_1"]);
	this.gotoAndStop(1);
}).prototype = p = new cjs.Sprite();



(lib.CO_AEP_300x2503 = function() {
	this.initialize(ss["CO_AEP_728x90_atlas_NP_1"]);
	this.gotoAndStop(2);
}).prototype = p = new cjs.Sprite();



(lib.CO_AEP_300x2504 = function() {
	this.initialize(ss["CO_AEP_728x90_atlas_NP_1"]);
	this.gotoAndStop(3);
}).prototype = p = new cjs.Sprite();
// helper functions:

function mc_symbol_clone() {
	var clone = this._cloneProps(new this.constructor(this.mode, this.startPosition, this.loop, this.reversed));
	clone.gotoAndStop(this.currentFrame);
	clone.paused = this.paused;
	clone.framerate = this.framerate;
	return clone;
}

function getMCSymbolPrototype(symbol, nominalBounds, frameBounds) {
	var prototype = cjs.extend(symbol, cjs.MovieClip);
	prototype.clone = mc_symbol_clone;
	prototype.nominalBounds = nominalBounds;
	prototype.frameBounds = frameBounds;
	return prototype;
	}


(lib.PIC4 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// pic
	this.instance = new lib.CO_AEP_300x2504();
	this.instance.setTransform(-150,0,0.5,0.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.PIC4, new cjs.Rectangle(-150,0,300,132), null);


(lib.PIC3 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// pic
	this.instance = new lib.CO_AEP_300x2503();
	this.instance.setTransform(-150,0,0.5,0.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.PIC3, new cjs.Rectangle(-150,0,300,132), null);


(lib.PIC2 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// pic
	this.instance = new lib.CO_AEP_300x2502();
	this.instance.setTransform(-150,0,0.5,0.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// mask
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("A3bKUIAA0nMAu3AAAIAAUng");
	this.shape.setTransform(0,66);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.PIC2, new cjs.Rectangle(-150,0,300,132), null);


(lib.PIC1 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// pic
	this.instance = new lib.CO_AEP_300x2501();
	this.instance.setTransform(-150,0,0.5,0.5);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.PIC1, new cjs.Rectangle(-150,0,300,132), null);


(lib.phone2rubinecircleai = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// phone2-rubine-circle.ai
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("");
	this.shape.setTransform(28.0908,26.125);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E40571").s().p("Ai/DAQhPhQAAhwQAAhvBPhQQBQhPBvAAQBwAABQBPQBPBQAABvQAABwhPBQQhQBPhwAAQhvAAhQhPg");
	this.shape_1.setTransform(27.125,27.125);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(0,0,54.3,54.3);


(lib.HEAD4 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1 copy
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AhuB9QgjgyAAhLQAAhKAjgzQAog5BGAAQBHAAAoA5QAjAzAABKQAABLgjAyQgoA6hHAAQhGAAgog6gAhEAAQAAB0BEgBQBFABAAh0QAAhzhFAAQhEAAAABzg");
	this.shape.setTransform(-88.075,37.25);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("AgUDmIAAgwQhNgHgwgwIApg6QAjAkAxAJIAAhSQg0gNgZgSQglgaAAgxQAAgpAfgdQAfgeA0gGIAAgxIAxAAIAAAyQBBAIAsAoIgqA4QgbgZgogKIAABJQA2AOAZASQAmAaAAAxQAAAugeAdQgeAgg5AFIAAAwgAAdBzQAogIAAgdQAAgWgogMgAg4hPQAAASAkAMIAAhAQgkAFAAAdg");
	this.shape_1.setTransform(-120.8,37.15);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_1},{t:this.shape}]}).wait(1));

	// Layer_1
	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AgoAiIALgSQAGAFAIAEQAJAEAIAAQAPAAAAgKQAAgEgIgDIgSgEQgbgHAAgTQAAgNAKgIQAKgIARAAQAUAAAQAMIgJARQgLgKgQAAQgGAAgEACQgEADAAAEQAAAEAIACIASAEQAcAHgBAVQAAANgKAIQgLAIgSAAQgZAAgQgOg");
	this.shape_2.setTransform(33.5,68.425);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("AggAjQgOgNAAgWQAAgTAOgOQANgOAUAAQAUAAANAOQANAOAAAVIAAAGIhDAAQABAJAHAFQAHAGAKAAQAPAAAJgJIALAQQgOANgYAAQgUAAgOgNgAAYgIQgBgIgFgFQgGgHgLAAQgKAAgGAHQgFAFgBAIIAtAAIAAAAg");
	this.shape_3.setTransform(23.825,68.425);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AgLA/IAAh9IAXAAIAAB9g");
	this.shape_4.setTransform(16.15,66.675);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AgWAyIAAAMIgXAAIAAh9IAXAAIAAAuQAMgOAQAAQASAAALANQALANAAAVQAAAWgLANQgLANgSABQgQAAgMgPgAgNgGQgFADgEADIAAAgQAEAFAFADQAGADAHAAQAJAAAHgHQAFgIAAgMQAAgMgFgGQgHgIgJABQgHAAgGADg");
	this.shape_5.setTransform(8.5,66.8);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("AgLBBIAAhbIAXAAIAABbgAgJgoQgFgEABgGQgBgGAFgEQAEgEAFAAQAGAAAEAEQAEAEABAGQgBAGgEAEQgEAFgGAAQgFAAgEgFg");
	this.shape_6.setTransform(0.25,66.475);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#FFFFFF").s().p("AgOAiIAAguIgQAAIAAgWIAQAAIAAgZIAXAAIAAAZIASAAIAAAWIgSAAIAAAoQAAAKAIAAQAGAAADgDIAFATQgHAGgNAAQgZAAAAgag");
	this.shape_7.setTransform(-5.325,67.275);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#FFFFFF").s().p("AgcAjQgNgOAAgVQAAgUANgOQAOgNAUAAQAYAAAMAQIgQAPQgGgKgNAAQgKAAgHAIQgGAHgBALQABAMAGAHQAHAIAKAAQANAAAGgKIAQAOQgMARgYAAQgUAAgOgNg");
	this.shape_8.setTransform(-13.15,68.425);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#FFFFFF").s().p("AgqASIAAhAIAYAAIAAA3QAAARARAAQALAAAJgLIAAg9IAYAAIAABbIgYAAIAAgMQgNAOgSAAQgeAAAAgdg");
	this.shape_9.setTransform(-23.5,68.525);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#FFFFFF").s().p("AgiAzQgMgNAAgWQAAgVAMgNQALgNASAAQARAAAKAOIAAguIAZAAIAAB9IgZAAIAAgMQgLAPgQAAQgSgBgLgNgAgOgCQgHAGAAAMQAAAMAHAIQAGAHAKAAQAGAAAFgDQAGgDADgFIAAggQgDgDgGgDQgFgDgGAAQgKgBgGAIg");
	this.shape_10.setTransform(-34.825,66.8);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#FFFFFF").s().p("AggAjQgOgNAAgWQAAgTAOgOQANgOAUAAQAUAAANAOQANAOAAAVIAAAGIhDAAQABAJAHAFQAHAGAKAAQAPAAAJgJIALAQQgOANgYAAQgUAAgOgNgAAYgIQgBgIgFgFQgGgHgLAAQgKAAgGAHQgFAFgBAIIAtAAIAAAAg");
	this.shape_11.setTransform(-45.325,68.425);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#FFFFFF").s().p("AgiAzQgMgNAAgWQAAgVAMgNQALgNASAAQARAAAKAOIAAguIAZAAIAAB9IgZAAIAAgMQgLAPgQAAQgSgBgLgNgAgOgCQgHAGAAAMQAAAMAHAIQAGAHAKAAQAGAAAFgDQAGgDADgFIAAggQgDgDgGgDQgFgDgGAAQgKgBgGAIg");
	this.shape_12.setTransform(-56.425,66.8);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#FFFFFF").s().p("AgiAzQgMgMAAgXQAAgVAMgNQALgNASAAQARAAAKAOIAAgvIAZAAIAAB+IgZAAIAAgMQgLAPgQAAQgSgBgLgNgAgOgCQgHAGAAAMQAAAMAHAIQAGAHAKAAQAGAAAFgDQAGgDADgFIAAggQgDgDgGgDQgFgDgGgBQgKAAgGAIg");
	this.shape_13.setTransform(30.175,47.8);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#FFFFFF").s().p("AATAvIAAg3QAAgRgRAAQgLAAgJALIAAA9IgYAAIAAhbIAYAAIAAAMQAMgOATAAQAeAAAAAdIAABAg");
	this.shape_14.setTransform(19.4,49.3);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#FFFFFF").s().p("AggApQgJgJgBgOQABgPAJgHQAJgHANAAQATAAAJALIAAgLQAAgHgGgFQgFgEgIAAQgPAAgLALIgKgRQAQgOAXAAQASAAAKAHQANAJAAASIAAA7IgZAAIAAgKQgKAMgSAAQgMAAgKgHgAgNAIQgEAEAAAGQAAAGAEAEQAGAEAHAAQAMAAAGgIIAAgMQgGgIgMAAQgHAAgGAEg");
	this.shape_15.setTransform(8.5,49.425);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#FFFFFF").s().p("AgoAiIALgSQAGAFAIAEQAJAEAIAAQAPAAAAgKQAAgEgJgDIgRgEQgcgHAAgTQABgNAJgIQALgIARAAQAUAAAQAMIgJARQgLgKgQAAQgGAAgEACQgEADAAAEQAAAEAJACIARAEQAbAHAAAVQAAANgKAIQgLAIgSAAQgZAAgQgOg");
	this.shape_16.setTransform(-5.85,49.425);

	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#FFFFFF").s().p("AgnA/IAEgVQADABAEABQAKAAADgIIADgIIglhbIAaAAIAXA/IAYg/IAaAAIgrBqQgFAMgIAFQgIAEgNAAQgHABgFgCg");
	this.shape_17.setTransform(-15.125,51.25);

	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#FFFFFF").s().p("AgfApQgKgJAAgOQAAgPAKgHQAIgHANAAQASAAAKALIAAgLQAAgHgFgFQgGgEgIAAQgPAAgMALIgJgRQAQgOAXAAQASAAAKAHQAMAJAAASIAAA7IgYAAIAAgKQgKAMgSAAQgMAAgJgHgAgMAIQgGAEAAAGQAAAGAGAEQAFAEAGAAQANAAAGgIIAAgMQgGgIgNAAQgGAAgFAEg");
	this.shape_18.setTransform(-25.4,49.425);

	this.shape_19 = new cjs.Shape();
	this.shape_19.graphics.f("#FFFFFF").s().p("AgtBBIAAh+IAYAAIAAALQAKgNARAAQASAAALAMQALANAAAXQAAAVgLANQgLANgSAAQgQAAgLgOIAAAvgAgMgnQgHADgCAEIAAAgQACAEAHADQAFADAHABQAJgBAGgHQAHgGgBgMQABgNgHgGQgGgIgJAAQgHAAgFADg");
	this.shape_19.setTransform(-35.5,51.05);

	this.shape_20 = new cjs.Shape();
	this.shape_20.graphics.f("#FFFFFF").s().p("AgiAiQgNgOgBgUQABgTANgOQANgOAVAAQAWAAAOAOQANAOAAATQAAAUgNAOQgOAOgWAAQgVAAgNgOgAgQgSQgHAIABAKQgBALAHAIQAGAIAKAAQALAAAGgIQAHgIgBgLQABgKgHgIQgGgIgLAAQgKAAgGAIg");
	this.shape_20.setTransform(-46.8,49.425);

	this.shape_21 = new cjs.Shape();
	this.shape_21.graphics.f("#FFFFFF").s().p("AgbAjQgOgOgBgVQABgUAOgOQANgNAUAAQAZAAAMAQIgQAPQgHgKgMAAQgLAAgGAIQgIAHABALQgBAMAIAHQAGAIALAAQALAAAIgKIAQAOQgMARgZAAQgUAAgNgNg");
	this.shape_21.setTransform(-56.85,49.425);

	this.shape_22 = new cjs.Shape();
	this.shape_22.graphics.f("#FFFFFF").s().p("AgNATQAEgDAFgFQAEgFABgFIgDAAQgFAAgFgCQgDgEAAgGQAAgHAFgEQAEgEAGAAQAGAAAEAFQAGAFAAAJQAAAUgTAOg");
	this.shape_22.setTransform(26.4,34.875);

	this.shape_23 = new cjs.Shape();
	this.shape_23.graphics.f("#FFFFFF").s().p("AgoAiIALgSQAGAFAIAEQAKAEAGAAQAQAAAAgKQAAgEgIgDIgSgEQgcgHABgTQgBgNALgIQAKgIARAAQAUAAAQAMIgKARQgKgKgQAAQgGAAgEACQgEADAAAEQAAAEAIACIASAEQAbAHAAAVQABANgLAIQgLAIgSAAQgZAAgQgOg");
	this.shape_23.setTransform(19.3,30.425);

	this.shape_24 = new cjs.Shape();
	this.shape_24.graphics.f("#FFFFFF").s().p("AAtAvIAAg5QAAgPgOAAQgLAAgIALIAAA9IgYAAIAAg5QAAgPgNAAQgLAAgIALIAAA9IgYAAIAAhbIAYAAIAAAMQAEgFAIgEQAIgFAKAAQATAAAGARQAEgHAIgFQAJgFAKAAQAbAAAAAcIAABBg");
	this.shape_24.setTransform(6.75,30.3);

	this.shape_25 = new cjs.Shape();
	this.shape_25.graphics.f("#FFFFFF").s().p("AgqASIAAhAIAYAAIAAA3QAAARARAAQALAAAJgLIAAg9IAYAAIAABbIgYAAIAAgMQgNAOgSAAQgeAAAAgdg");
	this.shape_25.setTransform(-6.85,30.525);

	this.shape_26 = new cjs.Shape();
	this.shape_26.graphics.f("#FFFFFF").s().p("AgLBBIAAhbIAXAAIAABbgAgJgoQgEgEgBgGQABgGAEgEQAEgEAFAAQAGAAAEAEQAFAEgBAGQABAGgFAEQgEAFgGAAQgFAAgEgFg");
	this.shape_26.setTransform(-14.75,28.475);

	this.shape_27 = new cjs.Shape();
	this.shape_27.graphics.f("#FFFFFF").s().p("AAtAvIAAg5QAAgPgOAAQgMAAgHALIAAA9IgXAAIAAg5QAAgPgOAAQgLAAgIALIAAA9IgYAAIAAhbIAYAAIAAAMQAEgFAHgEQAKgFAJAAQATAAAFARQAFgHAIgFQAJgFAKAAQAbAAAAAcIAABBg");
	this.shape_27.setTransform(-25.25,30.3);

	this.shape_28 = new cjs.Shape();
	this.shape_28.graphics.f("#FFFFFF").s().p("AggAjQgOgNAAgWQAAgTAOgOQANgOAUAAQAUAAANAOQANAOAAAVIAAAGIhDAAQABAJAHAFQAHAGAKAAQAPAAAJgJIALAQQgOANgYAAQgUAAgOgNgAAYgIQgBgIgFgFQgGgHgLAAQgKAAgGAHQgFAFgBAIIAtAAIAAAAg");
	this.shape_28.setTransform(-38.575,30.425);

	this.shape_29 = new cjs.Shape();
	this.shape_29.graphics.f("#FFFFFF").s().p("AgaAvIAAhbIAYAAIAAANQAMgPARAAIAAAYIgHgBQgGAAgHADQgHADgCAFIAAA7g");
	this.shape_29.setTransform(-46.775,30.3);

	this.shape_30 = new cjs.Shape();
	this.shape_30.graphics.f("#FFFFFF").s().p("AgtBBIAAh+IAXAAIAAAMQAMgPAQAAQASAAALANQALANAAAXQAAAVgLANQgLANgSAAQgQAAgMgOIAAAvgAgNgnQgFADgEAEIAAAgQAEAEAFADQAGADAHABQAJAAAGgIQAHgGgBgMQABgNgHgGQgGgIgJAAQgHAAgGADg");
	this.shape_30.setTransform(-55.85,32.05);

	this.shape_31 = new cjs.Shape();
	this.shape_31.graphics.f("#FFFFFF").s().p("AgnA/IAEgVQADACAEAAQAKgBADgGIADgIIglhcIAaAAIAXA/IAYg/IAaAAIgrBpQgFAMgIAGQgIAEgNABQgHgBgFgBg");
	this.shape_31.setTransform(32.225,13.25);

	this.shape_32 = new cjs.Shape();
	this.shape_32.graphics.f("#FFFFFF").s().p("AgLA/IAAh9IAXAAIAAB9g");
	this.shape_32.setTransform(24.95,9.675);

	this.shape_33 = new cjs.Shape();
	this.shape_33.graphics.f("#FFFFFF").s().p("AATA/IAAg4QAAgPgRAAQgMAAgHAJIAAA+IgZAAIAAh9IAZAAIAAAuQAMgOASAAQAeAAAAAdIAABAg");
	this.shape_33.setTransform(17.05,9.675);

	this.shape_34 = new cjs.Shape();
	this.shape_34.graphics.f("#FFFFFF").s().p("AgOAiIAAguIgQAAIAAgWIAQAAIAAgZIAXAAIAAAZIASAAIAAAWIgSAAIAAAoQAAAKAIAAQAGAAADgDIAFATQgHAGgNAAQgZAAAAgag");
	this.shape_34.setTransform(8.375,10.275);

	this.shape_35 = new cjs.Shape();
	this.shape_35.graphics.f("#FFFFFF").s().p("AATAvIAAg3QAAgQgRgBQgLAAgJALIAAA9IgYAAIAAhbIAYAAIAAAMQAMgOATAAQAeAAAAAdIAABAg");
	this.shape_35.setTransform(-0.35,11.3);

	this.shape_36 = new cjs.Shape();
	this.shape_36.graphics.f("#FFFFFF").s().p("AgjAiQgMgOAAgUQAAgTAMgOQAOgOAVAAQAWAAAOAOQAMAOAAATQAAAUgMAOQgOAOgWAAQgVAAgOgOgAgRgSQgFAIgBAKQABALAFAIQAHAIAKAAQALAAAHgIQAFgIABgLQgBgKgFgIQgHgIgLAAQgKAAgHAIg");
	this.shape_36.setTransform(-11.3,11.425);

	this.shape_37 = new cjs.Shape();
	this.shape_37.graphics.f("#FFFFFF").s().p("AAtAvIAAg5QAAgPgOAAQgMAAgHALIAAA9IgYAAIAAg5QAAgPgNAAQgLAAgIALIAAA9IgYAAIAAhbIAYAAIAAAMQAEgFAIgEQAJgFAJAAQATAAAGARQAEgHAIgFQAJgFAKAAQAbAAAAAcIAABBg");
	this.shape_37.setTransform(-24.85,11.3);

	this.shape_38 = new cjs.Shape();
	this.shape_38.graphics.f("#FFFFFF").s().p("AgaAvIAAhbIAYAAIAAAMQAMgOARAAIAAAXIgHAAQgGAAgHADQgHADgCAFIAAA7g");
	this.shape_38.setTransform(-40.725,11.3);

	this.shape_39 = new cjs.Shape();
	this.shape_39.graphics.f("#FFFFFF").s().p("AgjAiQgNgOAAgUQAAgTANgOQAOgOAVAAQAWAAAOAOQANAOAAATQAAAUgNAOQgOAOgWAAQgVAAgOgOgAgRgSQgFAIAAAKQAAALAFAIQAHAIAKAAQALAAAGgIQAHgIgBgLQABgKgHgIQgGgIgLAAQgKAAgHAIg");
	this.shape_39.setTransform(-50.05,11.425);

	this.shape_40 = new cjs.Shape();
	this.shape_40.graphics.f("#FFFFFF").s().p("AgTBAIAAhFIgPAAIAAgWIAPAAIAAgDQAAgQAJgJQAIgIANAAQAQAAAIAIIgJAPQgEgEgGAAQgMAAAAAOIAAADIATAAIAAAWIgTAAIAABFg");
	this.shape_40.setTransform(-57.975,9.575);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_40},{t:this.shape_39},{t:this.shape_38},{t:this.shape_37},{t:this.shape_36},{t:this.shape_35},{t:this.shape_34},{t:this.shape_33},{t:this.shape_32},{t:this.shape_31},{t:this.shape_30},{t:this.shape_29},{t:this.shape_28},{t:this.shape_27},{t:this.shape_26},{t:this.shape_25},{t:this.shape_24},{t:this.shape_23},{t:this.shape_22},{t:this.shape_21},{t:this.shape_20},{t:this.shape_19},{t:this.shape_18},{t:this.shape_17},{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2}]}).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.HEAD4, new cjs.Rectangle(-139,-1,268,80), null);


(lib.HEAD3 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AgNAOQgFgGAAgIQAAgHAFgGQAGgFAHAAQAIAAAGAFQAFAGAAAHQAAAIgFAGQgGAFgIAAQgHAAgGgFg");
	this.shape.setTransform(54.625,67.275);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("AgoAsQgSgQAAgcQAAgZARgRQARgSAZAAQAbAAAQASQAPARAAAcIAAAHIhVAAQACALAIAHQAJAIAOAAQASAAAMgLIAOAUQgRAQgfAAQgaAAgRgRgAAegKQgBgKgGgHQgIgIgOAAQgMAAgHAIQgHAHgBAKIA4AAIAAAAg");
	this.shape_1.setTransform(44.95,63.15);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AghA7IAAhzIAfAAIAAAQQAOgSAWAAIAAAdIgJgBQgHAAgJAFQgIAEgDAFIAABLg");
	this.shape_2.setTransform(34.575,63);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("AgsArQgRgRAAgaQAAgZARgRQARgSAbAAQAcAAARASQARARAAAZQAAAagRARQgRASgcAAQgbAAgRgSgAgVgWQgIAJABANQgBAOAIAKQAIAKANAAQANAAAJgKQAHgKAAgOQAAgOgHgIQgJgLgNAAQgNAAgIALg");
	this.shape_3.setTransform(22.8,63.15);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AA5A7IAAhIQAAgSgSAAQgOAAgKANIAABNIgdAAIAAhIQAAgSgSAAQgOAAgKANIAABNIgeAAIAAhzIAeAAIAAAQQAFgGAKgGQALgGAMAAQAZAAAGAUQAGgIAKgGQAMgGAMAAQAigBAAAkIAABSg");
	this.shape_4.setTransform(5.675,63);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AgrBCQgPgRAAgcQAAgcAPgQQAOgQAXAAQAVAAAOASIAAg8IAeAAIAACfIgeAAIAAgPQgPATgUgBQgXAAgOgPgAgSgDQgJAJAAAPQAAAOAJAKQAIAJAMAAQAIAAAHgDQAHgEAFgGIAAgpQgFgFgHgEQgHgDgIAAQgMAAgIAJg");
	this.shape_5.setTransform(-18.1,61.1);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("AAYA7IAAhFQAAgVgWAAQgPAAgKANIAABNIgeAAIAAhzIAeAAIAAAQQAQgSAYAAQAlgBABAlIAABRg");
	this.shape_6.setTransform(-31.65,63);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#FFFFFF").s().p("AgoA0QgNgMAAgRQAAgUANgJQALgIAQAAQAYAAAMAOIAAgPQAAgIgHgFQgHgGgKAAQgTAAgPAOIgMgWQAUgSAeAAQAWAAANAJQAQAMAAAXIAABKIgfAAIAAgMQgNAPgXAAQgPAAgMgJgAgQAKQgGAFAAAIQAAAIAGAEQAGAFAJAAQAQAAAIgKIAAgOQgIgKgQAAQgJgBgGAFg");
	this.shape_7.setTransform(-45.425,63.15);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#FFFFFF").s().p("AgyArIANgWQAHAGALAFQAMAFAKAAQATAAAAgNQAAgGgLgDIgXgFQgigIAAgZQAAgPANgLQANgLAVAAQAaAAAUAQIgMAWQgNgOgVAAQgIAAgFAEQgEACAAAGQgBAFALADIAXAFQAiAJAAAaQAAAQgNAKQgNALgYAAQgfAAgUgSg");
	this.shape_8.setTransform(62.1,39.15);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#FFFFFF").s().p("AghA7IAAhyIAfAAIAAAPQAOgTAWAAIAAAfIgJgCQgHABgJAEQgIADgDAGIAABLg");
	this.shape_9.setTransform(52.775,39);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#FFFFFF").s().p("AgoAsQgSgRAAgbQAAgYARgSQARgSAZAAQAbAAAPASQAQASAAAbIAAAHIhVAAQACALAIAHQAKAIAMAAQAUAAALgMIAOAVQgSAQgeAAQgaAAgRgRgAAegKQgBgKgGgHQgIgIgOAAQgMAAgIAIQgGAHgBAKIA4AAIAAAAg");
	this.shape_10.setTransform(41.3,39.15);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#FFFFFF").s().p("AgrBBQgPgQAAgdQAAgaAPgRQAOgRAWAAQAWABANARIAAg6IAfAAIAACeIgfAAIAAgOQgOARgVABQgWgBgOgQgAgSgDQgJAIAAAPQAAAQAJAJQAIAJAMAAQAIAAAHgEQAHgDAEgGIAAgpQgEgFgHgDQgHgEgIAAQgMAAgIAJg");
	this.shape_11.setTransform(27.25,37.1);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#FFFFFF").s().p("AgOBSIAAhzIAdAAIAABzgAgMgyQgFgGAAgHQAAgIAFgFQAGgFAGAAQAIAAAFAFQAFAFAAAIQAAAHgFAGQgFAFgIAAQgGAAgGgFg");
	this.shape_12.setTransform(17.575,36.725);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#FFFFFF").s().p("AgQA6IguhzIAgAAIAeBPIAehPIAhAAIguBzg");
	this.shape_13.setTransform(8.4,39.15);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#FFFFFF").s().p("AgsArQgRgSAAgZQAAgYARgSQARgSAbAAQAcAAARASQARASAAAYQAAAZgRASQgRASgcAAQgbAAgRgSgAgVgWQgIAJABANQgBAOAIAJQAIALANgBQANABAJgLQAHgJAAgOQAAgNgHgJQgJgLgNABQgNgBgIALg");
	this.shape_14.setTransform(-4.65,39.15);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#FFFFFF").s().p("AghA7IAAhyIAfAAIAAAPQAOgTAWAAIAAAfIgJgCQgHABgJAEQgIADgDAGIAABLg");
	this.shape_15.setTransform(-15.275,39);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#FFFFFF").s().p("Ag6BRIAAieIAfAAIAAAOQAOgRAVgBQAWABAOAPQAPARAAAcQAAAcgPAQQgOAQgWABQgVgBgOgRIAAA6gAgQgxQgHADgEAGIAAAoQAEAFAHAEQAHAEAIAAQAMAAAIgJQAJgIAAgQQAAgPgJgJQgIgJgMAAQgIAAgHAEg");
	this.shape_16.setTransform(-26.7,41.2);

	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#FFFFFF").s().p("AgYBRIAAhYIgTAAIAAgbIATAAIAAgEQAAgUALgLQALgLAQAAQAVAAAJALIgMASQgEgEgIAAQgPAAAAARIAAAEIAYAAIAAAbIgYAAIAABYg");
	this.shape_17.setTransform(-43.35,36.825);

	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#FFFFFF").s().p("AgsArQgRgSAAgZQAAgYARgSQARgSAbAAQAcAAARASQARASAAAYQAAAZgRASQgRASgcAAQgbAAgRgSgAgVgWQgIAJABANQgBAOAIAJQAIALANgBQANABAJgLQAHgJAAgOQAAgNgHgJQgJgLgNABQgNgBgIALg");
	this.shape_18.setTransform(-54.95,39.15);

	this.shape_19 = new cjs.Shape();
	this.shape_19.graphics.f("#FFFFFF").s().p("AASBQIgdgtIgOAPIAAAeIgfAAIAAifIAfAAIAABeIArgyIAlAAIguAzIAwBAg");
	this.shape_19.setTransform(80.525,12.95);

	this.shape_20 = new cjs.Shape();
	this.shape_20.graphics.f("#FFFFFF").s().p("AghA8IAAhzIAfAAIAAAPQAOgSAWAAIAAAeIgJgBQgHAAgJADQgIAFgDAFIAABMg");
	this.shape_20.setTransform(69.625,15);

	this.shape_21 = new cjs.Shape();
	this.shape_21.graphics.f("#FFFFFF").s().p("AgsArQgQgRgBgaQABgZAQgRQARgSAbAAQAcAAARASQAQARAAAZQAAAagQARQgRASgcAAQgbAAgRgSgAgVgXQgHAKgBANQABAPAHAIQAIALANAAQAOAAAIgLQAIgIAAgPQAAgNgIgKQgIgKgOAAQgNAAgIAKg");
	this.shape_21.setTransform(57.85,15.15);

	this.shape_22 = new cjs.Shape();
	this.shape_22.graphics.f("#FFFFFF").s().p("AAYA6IgYhOIgXBOIghAAIgkhzIAgAAIAWBNIAahNIAaAAIAZBNIAVhNIAhAAIgkBzg");
	this.shape_22.setTransform(41.75,15.15);

	this.shape_23 = new cjs.Shape();
	this.shape_23.graphics.f("#FFFFFF").s().p("AgSArIAAg7IgUAAIAAgbIAUAAIAAgfIAdAAIAAAfIAYAAIAAAbIgYAAIAAAzQAAANALAAQAHAAADgEIAHAYQgJAHgRAAQgfAAAAggg");
	this.shape_23.setTransform(28.475,13.725);

	this.shape_24 = new cjs.Shape();
	this.shape_24.graphics.f("#FFFFFF").s().p("AgoAtQgSgRAAgcQAAgZARgRQARgSAZAAQAbAAAPASQAQASAAAbIAAAHIhVAAQACALAIAHQAKAIAMAAQATAAAMgLIAOAUQgSAQgeAAQgaAAgRgQgAAegKQgBgKgGgGQgIgJgOAAQgMAAgIAIQgGAHgBAKIA4AAIAAAAg");
	this.shape_24.setTransform(17.8,15.15);

	this.shape_25 = new cjs.Shape();
	this.shape_25.graphics.f("#FFFFFF").s().p("AAYA8IAAhGQAAgVgVAAQgQAAgKANIAABOIgeAAIAAhzIAeAAIAAAPQAQgTAYABQAlAAABAlIAABRg");
	this.shape_25.setTransform(4.25,15);

	this.shape_26 = new cjs.Shape();
	this.shape_26.graphics.f("#FFFFFF").s().p("AgoAtQgSgRAAgcQAAgZARgRQARgSAZAAQAaAAARASQAPASAAAbIAAAHIhVAAQACALAIAHQAKAIANAAQATAAALgLIAOAUQgRAQgfAAQgaAAgRgQgAAegKQgBgKgGgGQgIgJgOAAQgMAAgHAIQgHAHgBAKIA4AAIAAAAg");
	this.shape_26.setTransform(-15.45,15.15);

	this.shape_27 = new cjs.Shape();
	this.shape_27.graphics.f("#FFFFFF").s().p("Ag1BCIAOgWQANAOAXAAQAMAAAJgGQALgHgBgRIAAgLQgOASgVAAQgWAAgOgPQgPgQAAgbQAAgcAPgPQAOgQAWAAQAVAAAOASIAAgPIAfAAIAABrQAAAegVAOQgQALgZAAQgeAAgUgRgAgSguQgJAIAAAPQAAAPAJAIQAIAIAMAAQAIAAAHgDQAHgFAEgEIAAglQgEgHgHgDQgHgDgIgBQgMAAgIAJg");
	this.shape_27.setTransform(-29.45,17.35);

	this.shape_28 = new cjs.Shape();
	this.shape_28.graphics.f("#FFFFFF").s().p("AghA8IAAhzIAfAAIAAAPQAOgSAWAAIAAAeIgJgBQgHAAgJADQgIAFgDAFIAABMg");
	this.shape_28.setTransform(-39.825,15);

	this.shape_29 = new cjs.Shape();
	this.shape_29.graphics.f("#FFFFFF").s().p("AgoA0QgNgLAAgTQAAgSANgKQALgIAQAAQAYAAAMAOIAAgPQAAgJgHgEQgHgGgKAAQgTAAgPAOIgMgWQAUgSAeAAQAWAAANAJQAQAMAAAXIAABKIgfAAIAAgMQgNAPgXAAQgPAAgMgJgAgQAKQgGAFAAAIQAAAIAGAFQAGAEAJAAQAQAAAIgKIAAgOQgIgLgQAAQgJAAgGAFg");
	this.shape_29.setTransform(-51.525,15.15);

	this.shape_30 = new cjs.Shape();
	this.shape_30.graphics.f("#FFFFFF").s().p("AgOBQIAAifIAdAAIAACfg");
	this.shape_30.setTransform(-60.725,12.95);

	this.shape_31 = new cjs.Shape();
	this.shape_31.graphics.f("#FFFFFF").s().p("AAtBQIgKgbIhEAAIgKAbIgnAAIA9ifIAqAAIA+CfgAAZAXIgZhEIgYBEIAxAAg");
	this.shape_31.setTransform(-78.1,12.95);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_31},{t:this.shape_30},{t:this.shape_29},{t:this.shape_28},{t:this.shape_27},{t:this.shape_26},{t:this.shape_25},{t:this.shape_24},{t:this.shape_23},{t:this.shape_22},{t:this.shape_21},{t:this.shape_20},{t:this.shape_19},{t:this.shape_18},{t:this.shape_17},{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.HEAD3, new cjs.Rectangle(-101.5,0,203,76), null);


(lib.HEAD2 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AgNAOQgFgGAAgIQAAgHAFgGQAGgFAHAAQAIAAAGAFQAFAGAAAHQAAAIgFAGQgGAFgIAAQgHAAgGgFg");
	this.shape.setTransform(71.675,67.275);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("AgyArIANgWQAHAGALAFQAMAFAKAAQATAAAAgMQAAgHgLgCIgWgGQgjgIAAgaQAAgPAMgKQAOgLAUAAQAbAAAUAQIgMAWQgNgOgWAAQgGAAgFADQgFAEgBAEQAAAGALADIAXAFQAiAJAAAaQAAARgNAKQgNAKgYAAQggAAgTgSg");
	this.shape_1.setTransform(62.8,63.15);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AgoAsQgSgQAAgcQAAgZARgRQARgSAZAAQAaAAARASQAPARAAAcIAAAHIhVAAQACALAIAHQAJAIANAAQATAAAMgLIAOAUQgRAQgfAAQgaAAgRgRgAAegKQgBgKgGgHQgIgIgOAAQgMAAgHAIQgHAHgBAKIA4AAIAAAAg");
	this.shape_2.setTransform(50.6,63.15);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("AgOBSIAAhzIAdAAIAABzgAgMgyQgFgGAAgHQAAgIAFgFQAGgFAGAAQAIAAAFAFQAFAFAAAIQAAAHgFAGQgFAFgIAAQgGAAgGgFg");
	this.shape_3.setTransform(40.925,60.725);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AghA7IAAhzIAfAAIAAAQQAOgSAWAAIAAAdIgJgBQgHAAgJAFQgIAEgDAFIAABLg");
	this.shape_4.setTransform(34.175,63);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AgoAsQgSgQAAgcQAAgZARgRQARgSAZAAQAaAAARASQAPARAAAcIAAAHIhVAAQACALAIAHQAJAIANAAQATAAAMgLIAOAUQgRAQgfAAQgaAAgRgRgAAegKQgBgKgGgHQgIgIgOAAQgMAAgHAIQgHAHgBAKIA4AAIAAAAg");
	this.shape_5.setTransform(22.7,63.15);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("AgjAsQgSgSAAgaQAAgZASgSQARgRAaAAQAeAAAQAVIgUATQgJgNgQAAQgNABgIAIQgJAKAAAOQAAAPAJAKQAIAJANAAQAQAAAJgNIAUATQgQAVgeAAQgaAAgRgRg");
	this.shape_6.setTransform(10.225,63.15);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#FFFFFF").s().p("AgsArQgQgRAAgaQAAgZAQgRQARgSAbAAQAcAAARASQAQARAAAZQAAAagQARQgRASgcAAQgbAAgRgSgAgVgWQgHAJgBANQABAOAHAKQAIAKANAAQAOAAAIgKQAIgKAAgOQAAgOgIgIQgIgLgOAAQgNAAgIALg");
	this.shape_7.setTransform(-2.8,63.15);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#FFFFFF").s().p("AghA7IAAhzIAfAAIAAAQQAOgSAWAAIAAAdIgJgBQgHAAgJAFQgIAEgDAFIAABLg");
	this.shape_8.setTransform(-13.425,63);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#FFFFFF").s().p("Ag0BCIANgWQANAOAXAAQAMAAAJgGQALgIAAgPIAAgMQgPASgUAAQgXAAgOgPQgPgQAAgbQAAgcAPgQQANgPAYAAQAUAAAPASIAAgPIAeAAIAABsQAAAdgVAOQgQALgZAAQgeAAgTgRgAgSguQgJAJAAAOQAAAPAJAIQAIAIAMAAQAHAAAIgEQAHgDAFgFIAAglQgFgGgHgEQgIgEgHAAQgMAAgIAJg");
	this.shape_9.setTransform(-25.65,65.35);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#FFFFFF").s().p("AghA7IAAhzIAfAAIAAAQQAOgSAWAAIAAAdIgJgBQgHAAgJAFQgIAEgDAFIAABLg");
	this.shape_10.setTransform(-42.175,63);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#FFFFFF").s().p("AgsArQgRgRAAgaQAAgZARgRQARgSAbAAQAcAAARASQARARAAAZQAAAagRARQgRASgcAAQgbAAgRgSgAgVgWQgHAJAAANQAAAOAHAKQAIAKANAAQANAAAJgKQAIgKgBgOQABgOgIgIQgJgLgNAAQgNAAgIALg");
	this.shape_11.setTransform(-53.95,63.15);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#FFFFFF").s().p("AgYBRIAAhYIgTAAIAAgbIATAAIAAgEQAAgUALgLQALgLAQAAQAVAAAJALIgLASQgFgEgIAAQgOAAAAARIAAAEIAXAAIAAAbIgXAAIAABYg");
	this.shape_12.setTransform(-63.95,60.825);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#FFFFFF").s().p("AghA7IAAhyIAfAAIAAAPQAOgTAWAAIAAAfIgJgCQgHABgJAEQgIADgDAGIAABLg");
	this.shape_13.setTransform(69.725,39);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#FFFFFF").s().p("AgoAzQgNgLAAgSQAAgSANgJQALgJAQAAQAYAAAMAOIAAgOQAAgJgHgGQgHgFgKAAQgTAAgPAOIgMgVQAUgTAeAAQAWAAANAKQAQALAAAXIAABKIgfAAIAAgMQgNAPgXAAQgPAAgMgKgAgQAKQgGAFAAAIQAAAIAGAEQAGAFAJAAQAQAAAIgKIAAgOQgIgLgQAAQgJABgGAEg");
	this.shape_14.setTransform(58.025,39.15);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#FFFFFF").s().p("AgoAsQgSgRAAgbQAAgYARgSQARgSAZAAQAbAAAPASQAQASAAAbIAAAHIhVAAQACALAIAHQAKAIAMAAQATAAAMgMIAOAVQgSAQgeAAQgaAAgRgRgAAegKQgBgKgGgHQgIgIgOAAQgMAAgIAIQgGAHgBAKIA4AAIAAAAg");
	this.shape_15.setTransform(45.25,39.15);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#FFFFFF").s().p("AgxBPIAEgbQAFADAEAAQANAAAEgJIAEgKIgvh0IAhAAIAdBQIAehQIAhAAIg3CGQgGAPgKAGQgKAGgRABQgJAAgFgDg");
	this.shape_16.setTransform(32.45,41.5);

	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#FFFFFF").s().p("AgoAzQgNgLAAgSQAAgSANgJQALgJAQAAQAYAAAMAOIAAgOQAAgJgHgGQgHgFgKAAQgTAAgPAOIgMgVQAUgTAeAAQAWAAANAKQAQALAAAXIAABKIgfAAIAAgMQgNAPgXAAQgPAAgMgKgAgQAKQgGAFAAAIQAAAIAGAEQAGAFAJAAQAQAAAIgKIAAgOQgIgLgQAAQgJABgGAEg");
	this.shape_17.setTransform(13.325,39.15);

	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#FFFFFF").s().p("AgxA5QgQgXAAgiQAAghAQgXQASgaAfAAQAgAAASAaQAQAXAAAhQAAAigQAXQgSAaggAAQgfAAgSgagAgeAAQAAA0AeAAQAgAAAAg0QAAgzggAAQgeAAAAAzg");
	this.shape_18.setTransform(-6.425,36.975);

	this.shape_19 = new cjs.Shape();
	this.shape_19.graphics.f("#FFFFFF").s().p("");
	this.shape_19.setTransform(-21.075,36.975);

	this.shape_20 = new cjs.Shape();
	this.shape_20.graphics.f("#FFFFFF").s().p("Ag+A5IATgYQARASAZAAQANAAAJgHQAHgGABgKQAAgLgIgHQgIgGgNgBQgTABgOANIgYgHIAAhaIBtAAIAAAeIhLAAIAAAmQAOgNAVAAQAVAAAPAOQAPANAAAYQAAAZgRAPQgSAPgcABQgnAAgXgZg");
	this.shape_20.setTransform(-35.45,37.1);

	this.shape_21 = new cjs.Shape();
	this.shape_21.graphics.f("#FFFFFF").s().p("AgRAYQAGgDAFgHQAGgHAAgGIgDAAQgHABgFgEQgEgFAAgHQAAgJAFgFQAGgGAIAAQAIABAGAFQAGAIAAALQAAAagXARg");
	this.shape_21.setTransform(-45.85,44.8);

	this.shape_22 = new cjs.Shape();
	this.shape_22.graphics.f("#FFFFFF").s().p("AAGBQIAAhzIgaAbIgTgUIAxgzIAeAAIAACfg");
	this.shape_22.setTransform(-54.575,36.95);

	this.shape_23 = new cjs.Shape();
	this.shape_23.graphics.f("#FFFFFF").s().p("AgIBoIAAgVQgjgDgWgWIATgbQAPARAXAEIAAglQgYgGgLgJQgRgLAAgWQAAgSAOgNQAOgOAYgDIAAgWIAVAAIAAAXQAeADATASIgTAaQgMgMgSgEIAAAhQAZAGALAIQARAMAAAWQAAAUgNAOQgOAOgaADIAAAVgAANA0QASgDAAgOQAAgJgSgGgAgZgjQAAAIARAFIAAgcQgRACAAANg");
	this.shape_23.setTransform(-66.225,36.925);

	this.shape_24 = new cjs.Shape();
	this.shape_24.graphics.f("#FFFFFF").s().p("AgSArIAAg7IgUAAIAAgbIAUAAIAAgfIAdAAIAAAfIAYAAIAAAbIgYAAIAAAzQAAANALAAQAHAAADgEIAHAYQgJAHgRAAQgfAAAAggg");
	this.shape_24.setTransform(59.725,13.725);

	this.shape_25 = new cjs.Shape();
	this.shape_25.graphics.f("#FFFFFF").s().p("AgoAtQgSgRAAgcQAAgZARgRQARgSAZAAQAaAAAQASQAQASAAAbIAAAHIhVAAQACALAIAHQAKAIAMAAQAUAAALgLIAOAUQgRAQgfAAQgaAAgRgQgAAegKQgBgKgGgGQgIgJgOAAQgMAAgIAIQgGAHgBAKIA4AAIAAAAg");
	this.shape_25.setTransform(49.05,15.15);

	this.shape_26 = new cjs.Shape();
	this.shape_26.graphics.f("#FFFFFF").s().p("Ag1BCIAOgWQANAOAXAAQANAAAIgGQAKgHAAgRIAAgLQgOASgVAAQgWAAgOgPQgPgQAAgbQAAgcAPgPQANgQAXAAQAWAAANASIAAgPIAfAAIAABrQAAAegVAOQgQALgZAAQgeAAgUgRgAgSguQgIAIAAAPQAAAPAIAIQAIAIAMAAQAHAAAIgDQAIgFADgEIAAglQgDgHgIgDQgIgDgHgBQgMAAgIAJg");
	this.shape_26.setTransform(35.05,17.35);

	this.shape_27 = new cjs.Shape();
	this.shape_27.graphics.f("#FFFFFF").s().p("AAYA8IAAhGQAAgVgWAAQgOAAgLANIAABOIgfAAIAAhzIAfAAIAAAPQAPgTAZABQAmAAgBAlIAABRg");
	this.shape_27.setTransform(15.35,15);

	this.shape_28 = new cjs.Shape();
	this.shape_28.graphics.f("#FFFFFF").s().p("AgoA0QgNgLAAgTQAAgSANgKQALgIAQAAQAYAAAMAOIAAgPQAAgJgHgEQgHgGgKAAQgTAAgPAOIgMgWQAUgSAeAAQAWAAANAJQAQAMAAAXIAABKIgfAAIAAgMQgNAPgXAAQgPAAgMgJgAgQAKQgGAFAAAIQAAAIAGAFQAGAEAJAAQAQAAAIgKIAAgOQgIgLgQAAQgJAAgGAFg");
	this.shape_28.setTransform(1.575,15.15);

	this.shape_29 = new cjs.Shape();
	this.shape_29.graphics.f("#FFFFFF").s().p("AgjAsQgSgSAAgaQAAgaASgQQARgSAaAAQAeAAAQAVIgUATQgJgNgQAAQgNAAgIAJQgJAKAAAOQAAAPAJAKQAIAIANABQAQAAAJgNIAUASQgQAWgeAAQgaAAgRgRg");
	this.shape_29.setTransform(-10.425,15.15);

	this.shape_30 = new cjs.Shape();
	this.shape_30.graphics.f("#FFFFFF").s().p("Ag1AXIAAhRIAeAAIAABFQAAAVAVAAQAQAAAKgNIAAhNIAfAAIAABzIgfAAIAAgPQgQARgYAAQgmAAABgkg");
	this.shape_30.setTransform(-29.6,15.3);

	this.shape_31 = new cjs.Shape();
	this.shape_31.graphics.f("#FFFFFF").s().p("AgsArQgQgRAAgaQAAgZAQgRQARgSAbAAQAcAAARASQAQARABAZQgBAagQARQgRASgcAAQgbAAgRgSgAgVgXQgIAKAAANQAAAPAIAIQAIALANAAQANAAAJgLQAHgIAAgPQAAgNgHgKQgJgKgNAAQgNAAgIAKg");
	this.shape_31.setTransform(-43.45,15.15);

	this.shape_32 = new cjs.Shape();
	this.shape_32.graphics.f("#FFFFFF").s().p("AgQBQIAAhCIg9hdIAnAAIAmBAIAohAIAmAAIg9BdIAABCg");
	this.shape_32.setTransform(-56.075,12.95);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_32},{t:this.shape_31},{t:this.shape_30},{t:this.shape_29},{t:this.shape_28},{t:this.shape_27},{t:this.shape_26},{t:this.shape_25},{t:this.shape_24},{t:this.shape_23},{t:this.shape_22},{t:this.shape_21},{t:this.shape_20},{t:this.shape_19},{t:this.shape_18},{t:this.shape_17},{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.HEAD2, new cjs.Rectangle(-110.5,0,221,76), null);


(lib.HEAD1 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AgMBNQgFgGAAgIQAAgHAFgHQAGgFAHAAQAIAAAGAFQAFAHAAAHQAAAIgFAGQgGAFgIABQgHgBgGgFgAgRAKQAAgPARgNIALgJQAGgFAAgGQAAgNgSAAQgRAAgNAPIgTgWQAUgXAfAAQAXgBAOALQANALAAARQAAATgUAPIgNAKQgHAGAAAGQAAAHAFADIgZAIQgIgIAAgNg");
	this.shape.setTransform(68.325,60.95);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("AgoAsQgSgQAAgcQAAgZARgRQARgSAZAAQAbAAAQASQAPARAAAcIAAAHIhVAAQACALAIAHQAJAIAOAAQASAAAMgLIAOAUQgSAQgeAAQgaAAgRgRgAAegKQgBgKgGgHQgIgIgOAAQgMAAgHAIQgHAHgBAKIA4AAIAAAAg");
	this.shape_1.setTransform(56.4,63.15);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AghA7IAAhzIAfAAIAAAQQAOgSAWAAIAAAdIgJgBQgHAAgJAFQgIAEgDAFIAABLg");
	this.shape_2.setTransform(46.025,63);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("AgoA0QgNgMAAgRQAAgUANgJQALgIAQAAQAYAAAMAOIAAgPQAAgIgHgFQgHgGgKAAQgTAAgPAOIgMgWQAUgSAeAAQAWAAANAJQAQAMAAAXIAABKIgfAAIAAgMQgNAPgXAAQgPAAgMgJgAgQAKQgGAFAAAIQAAAIAGAEQAGAFAJAAQAQAAAIgKIAAgOQgIgKgQAAQgJgBgGAFg");
	this.shape_3.setTransform(34.325,63.15);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AgjAsQgSgSAAgaQAAgZASgSQARgRAaAAQAeAAAQAVIgUATQgJgNgQAAQgNABgIAIQgJAKAAAOQAAAPAJAKQAIAJANAAQAQAAAJgNIAUATQgQAVgeAAQgaAAgRgRg");
	this.shape_4.setTransform(22.325,63.15);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AgOBSIAAhzIAdAAIAABzgAgMgyQgFgGAAgHQAAgIAFgFQAGgFAGAAQAIAAAFAFQAFAFAAAIQAAAHgFAGQgFAFgIAAQgGAAgGgFg");
	this.shape_5.setTransform(13.175,60.725);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("AgrBCQgPgRAAgcQAAgcAPgQQAOgQAWAAQAWAAANASIAAg8IAfAAIAACfIgfAAIAAgPQgOATgVgBQgWAAgOgPgAgSgDQgIAJgBAPQABAOAIAKQAIAJAMAAQAHAAAIgDQAIgEADgGIAAgpQgDgFgIgEQgIgDgHAAQgMAAgIAJg");
	this.shape_6.setTransform(2.75,61.1);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#FFFFFF").s().p("AgoAsQgSgQAAgcQAAgZARgRQARgSAZAAQAaAAARASQAPARAAAcIAAAHIhVAAQACALAIAHQAKAIANAAQATAAALgLIAOAUQgRAQgfAAQgaAAgRgRgAAegKQgBgKgGgHQgIgIgOAAQgMAAgHAIQgHAHgBAKIA4AAIAAAAg");
	this.shape_7.setTransform(-10.5,63.15);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#FFFFFF").s().p("AA0BQIAAhyIgtByIgOAAIgthyIAAByIgiAAIAAifIAwAAIAmBjIAnhjIAwAAIAACfg");
	this.shape_8.setTransform(-27.375,60.95);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#FFFFFF").s().p("AghA7IAAhzIAfAAIAAAQQAOgSAWAAIAAAdIgJgBQgHAAgJAFQgIAEgDAFIAABLg");
	this.shape_9.setTransform(-47.525,63);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#FFFFFF").s().p("AgsArQgQgRAAgaQAAgZAQgRQARgSAbAAQAcAAARASQAQARAAAZQAAAagQARQgRASgcAAQgbAAgRgSgAgVgWQgIAJAAANQAAAOAIAKQAIAKANAAQANAAAJgKQAHgKABgOQgBgOgHgIQgJgLgNAAQgNAAgIALg");
	this.shape_10.setTransform(-59.3,63.15);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#FFFFFF").s().p("AgYBRIAAhYIgTAAIAAgbIATAAIAAgEQAAgUALgLQAKgLARAAQAUAAAKALIgMASQgEgEgIAAQgPAAAAARIAAAEIAYAAIAAAbIgYAAIAABYg");
	this.shape_11.setTransform(-69.3,60.825);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#FFFFFF").s().p("AgoAsQgSgRAAgbQAAgYARgSQARgSAZAAQAbAAAPASQAQASAAAbIAAAHIhVAAQACALAIAHQAJAIAOAAQASAAAMgMIAOAVQgSAQgeAAQgaAAgRgRgAAegKQgBgKgGgHQgIgIgOAAQgMAAgIAIQgGAHgBAKIA4AAIAAAAg");
	this.shape_12.setTransform(84.75,39.15);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#FFFFFF").s().p("AgOBQIAAifIAdAAIAACfg");
	this.shape_13.setTransform(75.075,36.95);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#FFFFFF").s().p("AgcBAIAAAOIgeAAIAAieIAeAAIAAA6QAOgRAWgBQAWAAAOARQAPARAAAaQAAAdgPAQQgOAQgWABQgWgBgOgRgAgQgIQgIAEgEAEIAAApQAEAGAIADQAHAEAIAAQAMAAAIgJQAIgJAAgQQAAgPgIgIQgIgJgMAAQgIAAgHAEg");
	this.shape_14.setTransform(65.4,37.1);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#FFFFFF").s().p("AgOBSIAAhzIAdAAIAABzgAgMgyQgFgGAAgHQAAgIAFgFQAGgFAGAAQAIAAAFAFQAFAFAAAIQAAAHgFAGQgFAFgIAAQgGAAgGgFg");
	this.shape_15.setTransform(54.975,36.725);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#FFFFFF").s().p("Ag1BCIAOgWQANAOAXAAQAMAAAJgGQAKgHAAgRIAAgLQgOASgVAAQgWAAgOgPQgPgQAAgbQAAgbAPgRQAOgPAWAAQAWAAANASIAAgPIAfAAIAABrQAAAegVAOQgQALgZAAQgeAAgUgRgAgSguQgIAJgBAOQABAPAIAIQAIAIAMAAQAHAAAIgEQAIgEADgEIAAgmQgDgFgIgEQgIgEgHABQgMAAgIAIg");
	this.shape_16.setTransform(44.6,41.35);

	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#FFFFFF").s().p("AgOBSIAAhzIAdAAIAABzgAgMgyQgFgGAAgHQAAgIAFgFQAGgFAGAAQAIAAAFAFQAFAFAAAIQAAAHgFAGQgFAFgIAAQgGAAgGgFg");
	this.shape_17.setTransform(34.925,36.725);

	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#FFFFFF").s().p("AgOBQIAAifIAdAAIAACfg");
	this.shape_18.setTransform(28.875,36.95);

	this.shape_19 = new cjs.Shape();
	this.shape_19.graphics.f("#FFFFFF").s().p("AgoAsQgSgRAAgbQAAgYARgSQARgSAZAAQAbAAAPASQAQASAAAbIAAAHIhVAAQACALAIAHQAKAIAMAAQAUAAALgMIAOAVQgSAQgeAAQgaAAgRgRgAAegKQgBgKgGgHQgIgIgOAAQgMAAgIAIQgGAHgBAKIA4AAIAAAAg");
	this.shape_19.setTransform(19.25,39.15);

	this.shape_20 = new cjs.Shape();
	this.shape_20.graphics.f("#FFFFFF").s().p("AgrBBQgPgQAAgdQAAgaAPgRQAOgRAWAAQAWABANARIAAg6IAfAAIAACeIgfAAIAAgOQgOARgVABQgWgBgOgQgAgSgDQgIAIAAAPQAAAQAIAJQAIAJAMAAQAHAAAIgEQAIgDADgGIAAgpQgDgFgIgDQgIgEgHAAQgMAAgIAJg");
	this.shape_20.setTransform(-0.95,37.1);

	this.shape_21 = new cjs.Shape();
	this.shape_21.graphics.f("#FFFFFF").s().p("AAYA7IAAhEQAAgWgVAAQgQAAgKANIAABNIgfAAIAAhyIAfAAIAAAOQAPgRAYgBQAmABAAAkIAABRg");
	this.shape_21.setTransform(-14.5,39);

	this.shape_22 = new cjs.Shape();
	this.shape_22.graphics.f("#FFFFFF").s().p("AgoAzQgNgLAAgSQAAgSANgJQALgJAQAAQAYAAAMAOIAAgOQAAgJgHgGQgHgFgKAAQgTAAgPAOIgMgVQAUgTAeAAQAWAAANAKQAQALAAAXIAABKIgfAAIAAgMQgNAPgXAAQgPAAgMgKgAgQAKQgGAFAAAIQAAAIAGAEQAGAFAJAAQAQAAAIgKIAAgOQgIgLgQAAQgJABgGAEg");
	this.shape_22.setTransform(-28.275,39.15);

	this.shape_23 = new cjs.Shape();
	this.shape_23.graphics.f("#FFFFFF").s().p("Ag/BQIAAifIBJAAQAaAAAOAPQAOAOAAAXQAAAWgOANQgPAPgZAAIgnAAIAAA5gAgdgGIAjAAQAKAAAHgGQAGgGAAgJQAAgKgGgHQgHgFgKAAIgjAAg");
	this.shape_23.setTransform(-47.575,36.95);

	this.shape_24 = new cjs.Shape();
	this.shape_24.graphics.f("#FFFFFF").s().p("AAlBQIAAhEIhKAAIAABEIgiAAIAAifIAiAAIAAA/IBKAAIAAg/IAjAAIAACfg");
	this.shape_24.setTransform(-64.3,36.95);

	this.shape_25 = new cjs.Shape();
	this.shape_25.graphics.f("#FFFFFF").s().p("Ag7A8QgYgYAAgkQAAgjAYgYQAYgXAjAAQAkAAAYAXQAYAYAAAjQAAAkgYAYQgYAXgkAAQgjAAgYgXgAgigkQgOAOAAAWQAAAXAOAOQANAPAVABQAWgBAOgPQANgOAAgXQAAgWgNgOQgOgPgWgBQgVABgNAPg");
	this.shape_25.setTransform(-82.275,36.95);

	this.shape_26 = new cjs.Shape();
	this.shape_26.graphics.f("#FFFFFF").s().p("AAYA8IAAhGQAAgVgWAAQgOAAgLANIAABOIgeAAIAAhzIAeAAIAAAPQAPgTAZABQAmAAgBAlIAABRg");
	this.shape_26.setTransform(52.15,15);

	this.shape_27 = new cjs.Shape();
	this.shape_27.graphics.f("#FFFFFF").s().p("AgsArQgRgRAAgaQAAgZARgRQARgSAbAAQAcAAARASQARARAAAZQAAAagRARQgRASgcAAQgbAAgRgSgAgVgXQgHAKAAANQAAAPAHAIQAIALANAAQANAAAJgLQAIgIgBgPQABgNgIgKQgJgKgNAAQgNAAgIAKg");
	this.shape_27.setTransform(38.3,15.15);

	this.shape_28 = new cjs.Shape();
	this.shape_28.graphics.f("#FFFFFF").s().p("Ag2AXIAAhRIAfAAIAABFQAAAVAVAAQAPAAALgNIAAhNIAeAAIAABzIgeAAIAAgPQgQARgYAAQgmAAAAgkg");
	this.shape_28.setTransform(18.35,15.3);

	this.shape_29 = new cjs.Shape();
	this.shape_29.graphics.f("#FFFFFF").s().p("AgsArQgQgRAAgaQAAgZAQgRQARgSAbAAQAcAAARASQAQARAAAZQAAAagQARQgRASgcAAQgbAAgRgSgAgVgXQgIAKAAANQAAAPAIAIQAIALANAAQAOAAAIgLQAIgIAAgPQAAgNgIgKQgIgKgOAAQgNAAgIAKg");
	this.shape_29.setTransform(4.5,15.15);

	this.shape_30 = new cjs.Shape();
	this.shape_30.graphics.f("#FFFFFF").s().p("AgxBQIAEgbQAEABAFAAQANAAAEgIIAEgKIgvh0IAgAAIAeBQIAfhQIAgAAIg3CGQgFAPgLAGQgKAGgQAAQgJAAgGgBg");
	this.shape_30.setTransform(-8.55,17.5);

	this.shape_31 = new cjs.Shape();
	this.shape_31.graphics.f("#FFFFFF").s().p("AgoAtQgSgRAAgcQAAgZARgRQARgSAZAAQAaAAAQASQAQASAAAbIAAAHIhVAAQACALAIAHQAKAIAMAAQAUAAALgLIAOAUQgRAQgfAAQgaAAgRgQgAAegKQgBgKgGgGQgIgJgOAAQgMAAgIAIQgGAHgBAKIA4AAIAAAAg");
	this.shape_31.setTransform(-27.45,15.15);

	this.shape_32 = new cjs.Shape();
	this.shape_32.graphics.f("#FFFFFF").s().p("AghA8IAAhzIAfAAIAAAPQAOgSAWAAIAAAeIgJgBQgHAAgJADQgIAFgDAFIAABMg");
	this.shape_32.setTransform(-37.825,15);

	this.shape_33 = new cjs.Shape();
	this.shape_33.graphics.f("#FFFFFF").s().p("AAtBQIgKgbIhEAAIgLAbIgmAAIA+ifIApAAIA+CfgAAZAXIgZhEIgYBEIAxAAg");
	this.shape_33.setTransform(-50.9,12.95);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_33},{t:this.shape_32},{t:this.shape_31},{t:this.shape_30},{t:this.shape_29},{t:this.shape_28},{t:this.shape_27},{t:this.shape_26},{t:this.shape_25},{t:this.shape_24},{t:this.shape_23},{t:this.shape_22},{t:this.shape_21},{t:this.shape_20},{t:this.shape_19},{t:this.shape_18},{t:this.shape_17},{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.HEAD1, new cjs.Rectangle(-102.5,0,205,76), null);


(lib.coahreveps = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// coa-h-rev.eps
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("");
	this.shape.setTransform(361.0507,13.7);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#FFFFFF").s().p("");
	this.shape_1.setTransform(-246.3661,64.3125);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#FFFFFF").s().p("AiJCHQg2g5ABhRQAAhWA3g1QA3g1BXAAQBXAAAvAzQAzA2AABmIkqAAQATBwB6AAQBHAABOgtIAABIQgoAYglAKQgoALg0AAQheAAg6g9gAhDhqQgdAXgIAuIDUAAQgBgsgbgZQgagYgtAAQgvAAgdAYg");
	this.shape_2.setTransform(269.85,97.05);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#FFFFFF").s().p("");
	this.shape_3.setTransform(229.4,106.7);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#FFFFFF").s().p("AA6C7QgQgKgFgTQg3Amg7AAQgtAAgfgcQgcgbAAglQAAgyAvgeQAagQBHgWIBLgYIAAgUQAAgigTgRQgTgQgmAAQhLABg8A9IAAhRQAygzBcAAQBDAAApAeQAtAhAAA9IAADBQAAAUAQAAQAOgBAegTIAAAtQgeARgPAGQgRAGgTAAQgcAAgPgJgAg4AmQgoAXAAAdQAAAXARAQQARAOAaAAQAlAAAlgfIAAhuQhDAVgbAPg");
	this.shape_4.setTransform(191.3,97.05);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#FFFFFF").s().p("AgvDGQgjggAAg1IAAjGIhKAAIAAgIICdiJIAABOICGAAIAABDIiGAAIAADAQAAAeAVAPQAQAMAYAAQArAAA0gcIAABGQgWALgdAHQgfAHgUAAQhCAAgkghg");
	this.shape_5.setTransform(152.55,93.525);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#FFFFFF").s().p("ABeDAIAAjoQAAgqgWgZQgXgZgmAAQgaAAgZANQgYAMgeAeIAAENIhUAAIAAl4IBUAAIAAAyIACAAQAYgbAigPQAhgPAjAAQBAAAAmAlQArAoAABLIAADng");
	this.shape_6.setTransform(116.275,96.675);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#FFFFFF").s().p("AA6C7QgQgKgFgTQg3Amg7AAQgtAAgfgcQgcgbAAglQAAgyAvgeQAagQBHgWIBLgYIAAgUQgBgigTgRQgSgQgnAAQhKABg8A9IAAhRQAygzBcAAQBDAAApAeQAsAhABA9IAADBQAAAUAQAAQAOgBAegTIAAAtQgeARgPAGQgRAGgTAAQgcAAgPgJgAg4AmQgoAXAAAdQAAAXARAQQARAOAaAAQAlAAAlgfIAAhuQhCAVgcAPg");
	this.shape_7.setTransform(75.2,97.05);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#FFFFFF").s().p("AgLDAIjAl/IBcAAIBxDmIBwjmIBaAAIi7F/g");
	this.shape_8.setTransform(33.775,97.425);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#FFFFFF").s().p("AAOD5QheAAg4g2Qg3g1AAhbQgBhRA8g0QA7g1BcAAQArAAAuAPIAAiAIBUAAIAAHxgAhEggQgmAjAAA7QAAB8CCAAIBUAAIAAjvQgpgPglAAQg8AAgmAkg");
	this.shape_9.setTransform(-9.45,91.025);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#FFFFFF").s().p("AC6DyIhJiOIjlAAIhECOIhQAAIDpnjIA2AAIDyHjgAhWAmICnAAIhWitg");
	this.shape_10.setTransform(-54.775,91.7);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#FFFFFF").s().p("ABdDAIAAjoQABgqgXgYQgWgZgmgBQgaAAgYANQgZAMgcAeIAAENIhUAAIAAl4IBUAAIAAAyIABAAQAYgbAhgQQAigPAiAAQA/AAAmAmQAqApAABJIAADog");
	this.shape_11.setTransform(331.75,28.15);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#FFFFFF").s().p("AihCMQg/g5AAhWQAAhRBAg4QBAg3BeAAQBiAABBA2QBAA3AABTQAABZg/A3Qg/A3hlAAQhgAAg/g4gAhkhcQgnAjAAA2QAAA7AmAlQAlAkA9AAQBCAAAmgjQAngkgBg8QAAg3gogjQgogkg+AAQg7AAgmAkg");
	this.shape_12.setTransform(285.3,28.525);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#FFFFFF").s().p("");
	this.shape_13.setTransform(241.575,38.2);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#FFFFFF").s().p("AiICHQg1g5AAhSQAAhVA3g1QA3g1BWAAQBWAAAvAzQAyA2AABlIknAAQATByB5AAQBGAABNgtIAABIQgnAXglAKQgoALg0AAQhdAAg5g9gAhChqQgdAYgJAtIDTAAQgCgrgagZQgagZgsAAQguAAgdAYg");
	this.shape_14.setTransform(199.925,28.525);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#FFFFFF").s().p("AiZDAIAAl4IBUAAIAABXIACAAQA8hfA8AAQAxAAA0AsIgsBBQgTgQgXgLQgVgKgPAAQgqAAgdAiQgeAhAAAxIAADEg");
	this.shape_15.setTransform(166.55,28.15);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#FFFFFF").s().p("AjMCnQhLhEAAhjQAAhjBMhEQBQhIB7AAQB9AABPBIQBMBEAABjQAABjhLBEQhQBJh+AAQh8AAhPhJgAiQh/Qg6AzAABMQAABNA6AzQA5AzBXgBQBXABA6gzQA6gzAAhNQAAhMg6gzQg6gyhXAAQhXAAg5Ayg");
	this.shape_16.setTransform(116.7,24);

	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#FFFFFF").s().p("AiICHQg1g5AAhSQAAhVA3g1QA3g1BWAAQBWAAAvAzQAyA2AABlIknAAQATByB5AAQBGAABNgtIAABIQgnAXglAKQgnALg1AAQhdAAg5g9gAhChqQgdAYgJAtIDTAAQgCgrgagZQgagZgsAAQguAAgdAYg");
	this.shape_17.setTransform(62.775,28.525);

	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#FFFFFF").s().p("AiZDAIAAl4IBUAAIAABXIACAAQA8hfA8AAQAxAAAzAsIgsBBQgSgQgWgLQgWgKgPAAQgqAAgdAiQgeAhAAAxIAADEg");
	this.shape_18.setTransform(30,28.15);

	this.shape_19 = new cjs.Shape();
	this.shape_19.graphics.f("#FFFFFF").s().p("AA5C7QgQgJgFgUQg1Amg7AAQgtAAgegcQgcgaAAgmQAAgyAugeQAagRBHgVIBJgYIAAgUQAAgjgTgQQgSgQgmAAQhLAAg6A/IAAhSQAxgzBbAAQBCAAApAeQAsAhAAA9IAADBQAAAUAQAAQAPAAAdgUIAAAtQgdARgQAGQgQAGgTAAQgcAAgPgJgAg3AmQgoAWAAAeQAAAYARAPQARAOAaAAQAkAAAkgfIAAhuQhCAVgaAPg");
	this.shape_19.setTransform(-9.325,28.525);

	this.shape_20 = new cjs.Shape();
	this.shape_20.graphics.f("#FFFFFF").s().p("AinCgQhDhDAAhWQAAhqBRhGQBRhGB7AAQBLAABoAmIAABIQhegwhWAAQhZAAg6AzQg8AzAABMQAABPA7AxQA6AxBdAAQBcAABag0IAABJQgqAUgpAKQgxALg4AAQiGAAhQhQg");
	this.shape_20.setTransform(-56.9,24.45);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_20},{t:this.shape_19},{t:this.shape_18},{t:this.shape_17},{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

	this._renderFirstFrame();

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-367.1,0,734.3,135.2);


(lib.BKGDrubine = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#E40571").s().p("A3bTiMAAAgnDMAu3AAAMAAAAnDg");

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.BKGDrubine, new cjs.Rectangle(-150,-125,300,250), null);


(lib.LOGO = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Layer_1
	this.instance = new lib.coahreveps("synched",0);
	this.instance.setTransform(116.45,35.7,0.28,0.28,0,0,0,415.9,115.9);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.LOGO, new cjs.Rectangle(-102.7,3.3,205.60000000000002,37.800000000000004), null);


(lib.EndScreen = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	// Find out
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#E41C70").s().p("AgNAOQgFgGAAgIQAAgHAFgGQAGgFAHAAQAIAAAGAFQAFAGAAAHQAAAIgFAGQgGAFgIAAQgHAAgGgFg");
	this.shape.setTransform(-210.425,84.925);

	this.shape_1 = new cjs.Shape();
	this.shape_1.graphics.f("#E41C70").s().p("AgxBPIAEgbQAFACAFAAQAMABAEgJIAEgKIgvh1IAhAAIAdBRIAehRIAhAAIg2CGQgHAQgKAHQgKAFgRAAQgJAAgFgCg");
	this.shape_1.setTransform(-219.65,83.15);

	this.shape_2 = new cjs.Shape();
	this.shape_2.graphics.f("#E41C70").s().p("AgYBRIAAhYIgTAAIAAgbIATAAIAAgEQAAgUALgLQALgLAQAAQAVAAAJALIgLASQgFgEgIAAQgOAAAAARIAAAEIAXAAIAAAbIgXAAIAABYg");
	this.shape_2.setTransform(-228.9,78.475);

	this.shape_3 = new cjs.Shape();
	this.shape_3.graphics.f("#E41C70").s().p("AgOBSIAAhzIAdAAIAABzgAgMgyQgFgGAAgHQAAgIAFgFQAGgFAGAAQAIAAAFAFQAFAFAAAIQAAAHgFAGQgFAFgIAAQgGAAgGgFg");
	this.shape_3.setTransform(-236.625,78.375);

	this.shape_4 = new cjs.Shape();
	this.shape_4.graphics.f("#E41C70").s().p("AgOBQIAAifIAdAAIAACfg");
	this.shape_4.setTransform(-242.675,78.6);

	this.shape_5 = new cjs.Shape();
	this.shape_5.graphics.f("#E41C70").s().p("AgoAzQgNgLAAgRQAAgUANgIQALgJAQAAQAYAAAMAOIAAgOQAAgKgHgFQgHgFgKAAQgTAAgPAOIgMgVQAUgTAeAAQAWAAANAKQAQALAAAXIAABKIgfAAIAAgMQgNAPgXAAQgPAAgMgKgAgQAKQgGAFAAAIQAAAIAGAEQAGAFAJAAQAQAAAIgKIAAgOQgIgKgQAAQgJAAgGAEg");
	this.shape_5.setTransform(-252.525,80.8);

	this.shape_6 = new cjs.Shape();
	this.shape_6.graphics.f("#E41C70").s().p("Ag2AXIAAhSIAfAAIAABGQAAAVAVAAQAPAAALgNIAAhOIAeAAIAABzIgeAAIAAgOQgQARgYABQgmAAAAglg");
	this.shape_6.setTransform(-265.6,80.95);

	this.shape_7 = new cjs.Shape();
	this.shape_7.graphics.f("#E41C70").s().p("AAcBRIAAg6QgOASgVAAQgWAAgOgQQgPgRAAgbQAAgdAPgQQAOgQAWgBQAVABAOASIAAgQIAfAAIAACfgAgSgsQgIAJAAAQQAAAOAIAJQAIAJAMAAQAHAAAIgEQAHgDAEgGIAAgoQgEgGgHgEQgIgDgHAAQgMAAgIAJg");
	this.shape_7.setTransform(-279.875,82.85);

	this.shape_8 = new cjs.Shape();
	this.shape_8.graphics.f("#E41C70").s().p("Ag2AXIAAhSIAfAAIAABGQAAAVAWAAQAPAAAKgNIAAhOIAeAAIAABzIgeAAIAAgOQgQARgYABQgmAAAAglg");
	this.shape_8.setTransform(-299.55,80.95);

	this.shape_9 = new cjs.Shape();
	this.shape_9.graphics.f("#E41C70").s().p("AgsArQgQgSgBgZQABgZAQgRQARgSAbAAQAcAAARASQAQARAAAZQAAAZgQASQgRASgcAAQgbAAgRgSgAgVgWQgHAJgBANQABAOAHAKQAIAKANAAQAOAAAIgKQAIgKAAgOQAAgOgIgIQgIgKgOAAQgNAAgIAKg");
	this.shape_9.setTransform(-313.4,80.8);

	this.shape_10 = new cjs.Shape();
	this.shape_10.graphics.f("#E41C70").s().p("AgxBPIAEgbQAFACAEAAQANABAEgJIAEgKIgvh1IAhAAIAdBRIAfhRIAgAAIg3CGQgFAQgLAHQgKAFgRAAQgIAAgGgCg");
	this.shape_10.setTransform(-326.45,83.15);

	this.shape_11 = new cjs.Shape();
	this.shape_11.graphics.f("#E41C70").s().p("AgYBRIAAhYIgTAAIAAgbIATAAIAAgEQAAgUALgLQALgLAQAAQAVAAAJALIgLASQgFgEgIAAQgOAAAAARIAAAEIAXAAIAAAbIgXAAIAABYg");
	this.shape_11.setTransform(-221.35,54.475);

	this.shape_12 = new cjs.Shape();
	this.shape_12.graphics.f("#E41C70").s().p("AgOBSIAAhzIAdAAIAABzgAgMgyQgFgGAAgHQAAgIAFgFQAGgFAGAAQAIAAAFAFQAFAFAAAIQAAAHgFAGQgFAFgIAAQgGAAgGgFg");
	this.shape_12.setTransform(-229.075,54.375);

	this.shape_13 = new cjs.Shape();
	this.shape_13.graphics.f("#E41C70").s().p("AgSArIAAg7IgUAAIAAgbIAUAAIAAgfIAdAAIAAAfIAYAAIAAAbIgYAAIAAAzQAAANALAAQAHAAADgEIAHAYQgJAHgRAAQgfAAAAggg");
	this.shape_13.setTransform(-242.275,55.375);

	this.shape_14 = new cjs.Shape();
	this.shape_14.graphics.f("#E41C70").s().p("Ag1AXIAAhRIAeAAIAABFQAAAVAVAAQAQAAAKgNIAAhNIAfAAIAABzIgfAAIAAgPQgQASgYAAQgmAAABglg");
	this.shape_14.setTransform(-253.25,56.95);

	this.shape_15 = new cjs.Shape();
	this.shape_15.graphics.f("#E41C70").s().p("AgsArQgQgRAAgaQAAgYAQgSQARgSAbAAQAcAAARASQAQASABAYQgBAagQARQgRASgcAAQgbAAgRgSgAgVgWQgIAJAAANQAAAPAIAIQAIAKANAAQANAAAJgKQAHgIAAgPQAAgNgHgJQgJgLgNABQgNgBgIALg");
	this.shape_15.setTransform(-267.1,56.8);

	this.shape_16 = new cjs.Shape();
	this.shape_16.graphics.f("#E41C70").s().p("AgrBBQgPgQAAgdQAAgbAPgQQAOgRAXAAQAVAAAOASIAAg6IAeAAIAACfIgeAAIAAgPQgPARgUABQgXgBgOgQgAgSgDQgJAJAAAOQAAAQAJAJQAIAJAMAAQAHAAAIgEQAHgDAFgGIAAgpQgFgFgHgDQgIgEgHAAQgMAAgIAJg");
	this.shape_16.setTransform(-287.55,54.75);

	this.shape_17 = new cjs.Shape();
	this.shape_17.graphics.f("#E41C70").s().p("AAYA8IAAhFQAAgWgWAAQgOAAgLANIAABOIgeAAIAAhzIAeAAIAAAOQAPgSAZAAQAmABgBAkIAABSg");
	this.shape_17.setTransform(-301.1,56.65);

	this.shape_18 = new cjs.Shape();
	this.shape_18.graphics.f("#E41C70").s().p("AgOBSIAAhzIAdAAIAABzgAgMgyQgFgGAAgHQAAgIAFgFQAGgFAGAAQAIAAAFAFQAFAFAAAIQAAAHgFAGQgFAFgIAAQgGAAgGgFg");
	this.shape_18.setTransform(-311.075,54.375);

	this.shape_19 = new cjs.Shape();
	this.shape_19.graphics.f("#E41C70").s().p("Ag3BQIAAifIBvAAIAAAeIhOAAIAAAhIBNAAIAAAeIhNAAIAABCg");
	this.shape_19.setTransform(-320.55,54.6);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_19},{t:this.shape_18},{t:this.shape_17},{t:this.shape_16},{t:this.shape_15},{t:this.shape_14},{t:this.shape_13},{t:this.shape_12},{t:this.shape_11},{t:this.shape_10},{t:this.shape_9},{t:this.shape_8},{t:this.shape_7},{t:this.shape_6},{t:this.shape_5},{t:this.shape_4},{t:this.shape_3},{t:this.shape_2},{t:this.shape_1},{t:this.shape}]}).wait(1));

	// tty
	this.shape_20 = new cjs.Shape();
	this.shape_20.graphics.f("#335192").s().p("AAEAyIAAhIIgQARIgMgMIAeggIATAAIAABjg");
	this.shape_20.setTransform(84.275,35.5);

	this.shape_21 = new cjs.Shape();
	this.shape_21.graphics.f("#335192").s().p("AAEAyIAAhIIgQARIgMgMIAeggIATAAIAABjg");
	this.shape_21.setTransform(78.125,35.5);

	this.shape_22 = new cjs.Shape();
	this.shape_22.graphics.f("#335192").s().p("AgWAyIAjhRIgxAAIAAgSIBKAAIAAAPIglBUg");
	this.shape_22.setTransform(71.4,35.5);

	this.shape_23 = new cjs.Shape();
	this.shape_23.graphics.f("#335192").s().p("AgKAyIAAgpIgmg6IAYAAIAYAoIAZgoIAYAAIgnA6IAAApg");
	this.shape_23.setTransform(58.65,35.5);

	this.shape_24 = new cjs.Shape();
	this.shape_24.graphics.f("#335192").s().p("AgJAyIAAhRIgeAAIAAgSIBPAAIAAASIgdAAIAABRg");
	this.shape_24.setTransform(49.45,35.5);

	this.shape_25 = new cjs.Shape();
	this.shape_25.graphics.f("#335192").s().p("AgKAyIAAhRIgdAAIAAgSIBPAAIAAASIgdAAIAABRg");
	this.shape_25.setTransform(40.75,35.5);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_25},{t:this.shape_24},{t:this.shape_23},{t:this.shape_22},{t:this.shape_21},{t:this.shape_20}]}).wait(1));

	// phone no
	this.shape_26 = new cjs.Shape();
	this.shape_26.graphics.f("#003C71").s().p("AgoAtQgSgSAAgbQAAgYARgSQARgSAZAAQAbAAAPASQAQASAAAbIAAAHIhVAAQACALAIAHQAKAIAMAAQAUAAALgMIAOAVQgSAQgeAAQgaAAgRgQgAAegKQgBgKgGgGQgIgJgOAAQgMAAgIAIQgGAHgBAKIA4AAIAAAAg");
	this.shape_26.setTransform(76.4,92.8);

	this.shape_27 = new cjs.Shape();
	this.shape_27.graphics.f("#003C71").s().p("AghA8IAAhzIAfAAIAAAPQAOgSAWgBIAAAfIgJgBQgHAAgJADQgIAEgDAGIAABMg");
	this.shape_27.setTransform(66.025,92.65);

	this.shape_28 = new cjs.Shape();
	this.shape_28.graphics.f("#003C71").s().p("AgsArQgQgRgBgaQABgYAQgSQARgSAbAAQAcAAARASQAQASAAAYQAAAagQARQgRASgcAAQgbAAgRgSgAgVgXQgHAKgBANQABAPAHAIQAIAKANAAQAOAAAIgKQAIgIAAgPQAAgNgIgKQgIgKgOABQgNgBgIAKg");
	this.shape_28.setTransform(54.25,92.8);

	this.shape_29 = new cjs.Shape();
	this.shape_29.graphics.f("#003C71").s().p("AA5A8IAAhJQAAgSgSAAQgOAAgKANIAABOIgdAAIAAhJQAAgSgSAAQgOAAgKANIAABOIgeAAIAAhzIAeAAIAAAOQAFgFAKgGQALgHAMAAQAZAAAGAWQAGgJAKgGQAMgHAMAAQAiABAAAiIAABUg");
	this.shape_29.setTransform(37.125,92.65);

	this.shape_30 = new cjs.Shape();
	this.shape_30.graphics.f("#003C71").s().p("AAYA8IAAhFQAAgWgVAAQgQAAgKANIAABOIgfAAIAAhzIAfAAIAAAOQAPgSAYAAQAmABAAAkIAABSg");
	this.shape_30.setTransform(13.85,92.65);

	this.shape_31 = new cjs.Shape();
	this.shape_31.graphics.f("#003C71").s().p("AghA8IAAhzIAfAAIAAAPQAOgSAWgBIAAAfIgJgBQgHAAgJADQgIAEgDAGIAABMg");
	this.shape_31.setTransform(3.175,92.65);

	this.shape_32 = new cjs.Shape();
	this.shape_32.graphics.f("#003C71").s().p("AgoAzQgNgKAAgTQAAgSANgJQALgJAQAAQAYAAAMAOIAAgOQAAgJgHgGQgHgFgKAAQgTAAgPAOIgMgVQAUgTAeAAQAWAAANAKQAQALAAAXIAABKIgfAAIAAgMQgNAPgXAAQgPAAgMgKgAgQAKQgGAFAAAIQAAAIAGAFQAGAEAJAAQAQAAAIgKIAAgOQgIgLgQAAQgJABgGAEg");
	this.shape_32.setTransform(-8.525,92.8);

	this.shape_33 = new cjs.Shape();
	this.shape_33.graphics.f("#003C71").s().p("AgoAtQgSgSAAgbQAAgYARgSQARgSAZAAQAaAAARASQAPASAAAbIAAAHIhVAAQACALAIAHQAKAIANAAQATAAALgMIAOAVQgRAQgfAAQgaAAgRgQgAAegKQgBgKgGgGQgIgJgOAAQgMAAgHAIQgHAHgBAKIA4AAIAAAAg");
	this.shape_33.setTransform(-21.3,92.8);

	this.shape_34 = new cjs.Shape();
	this.shape_34.graphics.f("#003C71").s().p("AgOBQIAAifIAdAAIAACfg");
	this.shape_34.setTransform(-30.975,90.6);

	this.shape_35 = new cjs.Shape();
	this.shape_35.graphics.f("#003C71").s().p("AgsArQgQgRAAgaQAAgYAQgSQARgSAbAAQAcAAARASQAQASABAYQgBAagQARQgRASgcAAQgbAAgRgSgAgVgXQgIAKAAANQAAAPAIAIQAIAKANAAQANAAAJgKQAHgIAAgPQAAgNgHgKQgJgKgNABQgNgBgIAKg");
	this.shape_35.setTransform(-47.05,92.8);

	this.shape_36 = new cjs.Shape();
	this.shape_36.graphics.f("#003C71").s().p("AgSArIAAg7IgUAAIAAgbIAUAAIAAgfIAdAAIAAAfIAYAAIAAAbIgYAAIAAAzQAAANALAAQAHAAADgEIAHAYQgJAHgRAAQgfAAAAggg");
	this.shape_36.setTransform(-57.975,91.375);

	this.shape_37 = new cjs.Shape();
	this.shape_37.graphics.f("#003C71").s().p("AASBQIgdgtIgOAPIAAAeIgfAAIAAifIAfAAIAABfIArgzIAlAAIguA0IAwA/g");
	this.shape_37.setTransform(-73.875,90.6);

	this.shape_38 = new cjs.Shape();
	this.shape_38.graphics.f("#003C71").s().p("AgjAsQgSgRAAgbQAAgaASgQQARgSAaAAQAeAAAQAVIgUATQgJgMgQAAQgNgBgIAKQgJAJAAAOQAAAPAJAJQAIAJANAAQAQAAAJgMIAUASQgQAWgeAAQgaAAgRgRg");
	this.shape_38.setTransform(-86.875,92.8);

	this.shape_39 = new cjs.Shape();
	this.shape_39.graphics.f("#003C71").s().p("AgOBSIAAhzIAdAAIAABzgAgMgyQgFgGAAgHQAAgIAFgFQAGgFAGAAQAIAAAFAFQAFAFAAAIQAAAHgFAGQgFAFgIAAQgGAAgGgFg");
	this.shape_39.setTransform(-96.025,90.375);

	this.shape_40 = new cjs.Shape();
	this.shape_40.graphics.f("#003C71").s().p("AgOBQIAAifIAdAAIAACfg");
	this.shape_40.setTransform(-102.075,90.6);

	this.shape_41 = new cjs.Shape();
	this.shape_41.graphics.f("#003C71").s().p("AgjAsQgSgRAAgbQAAgaASgQQARgSAaAAQAeAAAQAVIgUATQgJgMgQAAQgNgBgIAKQgJAJAAAOQAAAPAJAJQAIAJANAAQAQAAAJgMIAUASQgQAWgeAAQgaAAgRgRg");
	this.shape_41.setTransform(-110.925,92.8);

	this.shape_42 = new cjs.Shape();
	this.shape_42.graphics.f("#003C71").s().p("AghA8IAAhzIAfAAIAAAPQAOgSAWgBIAAAfIgJgBQgHAAgJADQgIAEgDAGIAABMg");
	this.shape_42.setTransform(-126.925,92.65);

	this.shape_43 = new cjs.Shape();
	this.shape_43.graphics.f("#003C71").s().p("AgsArQgQgRAAgaQAAgYAQgSQARgSAbAAQAcAAARASQAQASAAAYQAAAagQARQgRASgcAAQgbAAgRgSgAgVgXQgHAKgBANQABAPAHAIQAIAKANAAQAOAAAIgKQAIgIAAgPQAAgNgIgKQgIgKgOABQgNgBgIAKg");
	this.shape_43.setTransform(-138.7,92.8);

	this.shape_44 = new cjs.Shape();
	this.shape_44.graphics.f("#E41C70").s().p("Ag2B4IBVjCIh4AAIAAgtICzAAIAAAkIhZDLg");
	this.shape_44.setTransform(78.175,60.1);

	this.shape_45 = new cjs.Shape();
	this.shape_45.graphics.f("#E41C70").s().p("AhJBYQgYgiAAg2QAAg3AdghQAfgjAzAAQAqAAAdAZIgXAnQgUgTgcAAQgZAAgRAUQgRATAAAdIAAAFQAJgNAQgIQASgKASAAQAiAAAXAUQAZAVAAAkQAAAkgaAXQgaAYgoAAQgzAAgcgkgAgtAeQACAUALANQANAQAWAAQATAAANgMQALgLAAgNQAAgRgOgKQgMgJgTAAQgcAAgSAXg");
	this.shape_45.setTransform(57.475,60.15);

	this.shape_46 = new cjs.Shape();
	this.shape_46.graphics.f("#E41C70").s().p("AhLBVQgXgiAAgzQAAgyAXgjQAcgmAvAAQAwAAAbAmQAYAjAAAyQAAAzgYAiQgbAngwAAQgvAAgcgngAguAAQAABPAuAAQAvAAAAhPQAAhOgvAAQguAAAABOg");
	this.shape_46.setTransform(35.325,60.15);

	this.shape_47 = new cjs.Shape();
	this.shape_47.graphics.f("#E41C70").s().p("AhLBVQgXgiAAgzQAAgyAXgjQAcgmAvAAQAwAAAbAmQAYAjAAAyQAAAzgYAiQgbAngwAAQgvAAgcgngAguAAQAABPAuAAQAvAAAAhPQAAhOgvAAQguAAAABOg");
	this.shape_47.setTransform(13.025,60.15);

	this.shape_48 = new cjs.Shape();
	this.shape_48.graphics.f("#E41C70").s().p("AgqATIAAglIBVAAIAAAlg");
	this.shape_48.setTransform(-3.575,63.4);

	this.shape_49 = new cjs.Shape();
	this.shape_49.graphics.f("#E41C70").s().p("AhXB6IAAgpQBOg5AWgWQAbgZAAgVQAAgPgNgJQgKgIgRAAQglAAgaAcIgdgiQAignA6AAQAnAAAaAUQAaAWAAAjQAAAdgYAdQgWAbgxAkIBhAAIAAAtg");
	this.shape_49.setTransform(-20.1,59.925);

	this.shape_50 = new cjs.Shape();
	this.shape_50.graphics.f("#E41C70").s().p("AhdBWIAcgkQAZAbAnAAQAUAAAMgLQAMgKAAgPQAAgQgMgKQgLgKgUAAQgdAAgVAUIgkgKIAAiIICkAAIAAAtIhwAAIAAA6QAUgUAfAAQAgAAAWAVQAXAVAAAjQAAAmgaAWQgaAXgrAAQg7AAghgkg");
	this.shape_50.setTransform(-41.225,60.325);

	this.shape_51 = new cjs.Shape();
	this.shape_51.graphics.f("#E41C70").s().p("AhUBjIAXgnQAUATAcgBQAbAAARgUQAPgUAAgbIAAgFQgJAMgQAJQgSAJgSAAQgiABgXgVQgZgUAAgkQAAgkAagYQAagXAoAAQAzAAAcAlQAYAhAAA1QAAA3geAiQgeAjgzAAQgrAAgcgZgAgihCQgLAKAAANQAAATAOAJQAMAJASgBQAcAAATgWQgCgUgLgNQgNgPgWAAQgTgBgNAMg");
	this.shape_51.setTransform(-63.375,60.1);

	this.shape_52 = new cjs.Shape();
	this.shape_52.graphics.f("#E41C70").s().p("AgqATIAAglIBVAAIAAAlg");
	this.shape_52.setTransform(-79.675,63.4);

	this.shape_53 = new cjs.Shape();
	this.shape_53.graphics.f("#E41C70").s().p("AhJBYQgYgiAAg2QAAg3AdghQAfgjAzAAQAqAAAdAZIgXAnQgUgTgcAAQgZAAgRAUQgRATAAAdIAAAFQAJgNAQgIQASgKASAAQAiAAAXAUQAZAVAAAkQAAAkgaAXQgaAYgoAAQgzAAgcgkgAgtAeQACAUALANQANAQAWAAQATAAANgMQALgLAAgNQAAgRgOgKQgMgJgTAAQgcAAgSAXg");
	this.shape_53.setTransform(-95.875,60.15);

	this.shape_54 = new cjs.Shape();
	this.shape_54.graphics.f("#E41C70").s().p("AhJBYQgYgiAAg2QAAg3AdghQAfgjAzAAQAqAAAdAZIgXAnQgUgTgcAAQgZAAgRAUQgRATAAAdIAAAFQAJgNAQgIQASgKASAAQAiAAAXAUQAZAVAAAkQAAAkgaAXQgaAYgoAAQgzAAgcgkgAgtAeQACAUALANQANAQAWAAQATAAANgMQALgLAAgNQAAgRgOgKQgMgJgTAAQgcAAgSAXg");
	this.shape_54.setTransform(-117.725,60.15);

	this.shape_55 = new cjs.Shape();
	this.shape_55.graphics.f("#E41C70").s().p("");
	this.shape_55.setTransform(-139.65,60.15);

	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.shape_55},{t:this.shape_54},{t:this.shape_53},{t:this.shape_52},{t:this.shape_51},{t:this.shape_50},{t:this.shape_49},{t:this.shape_48},{t:this.shape_47},{t:this.shape_46},{t:this.shape_45},{t:this.shape_44},{t:this.shape_43},{t:this.shape_42},{t:this.shape_41},{t:this.shape_40},{t:this.shape_39},{t:this.shape_38},{t:this.shape_37},{t:this.shape_36},{t:this.shape_35},{t:this.shape_34},{t:this.shape_33},{t:this.shape_32},{t:this.shape_31},{t:this.shape_30},{t:this.shape_29},{t:this.shape_28},{t:this.shape_27},{t:this.shape_26}]}).wait(1));

	// phone icon
	this.instance = new lib.phone2rubinecircleai("synched",0);
	this.instance.setTransform(-433.35,67.85,1.5,1.5,0,0,0,27.1,27.1);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(1));

	// mask
	this.shape_56 = new cjs.Shape();
	this.shape_56.graphics.f("#FFFFFF").s().p("EgzEAJ3IAAztMBmJAAAIAATtg");
	this.shape_56.setTransform(-176.9,68.925);

	this.timeline.addTween(cjs.Tween.get(this.shape_56).wait(1));

	this._renderFirstFrame();

}).prototype = getMCSymbolPrototype(lib.EndScreen, new cjs.Rectangle(-503.8,5.8,653.8,126.3), null);


// stage content:
(lib.CO_AEP_728x90 = function(mode,startPosition,loop,reversed) {
if (loop == null) { loop = true; }
if (reversed == null) { reversed = false; }
	var props = new Object();
	props.mode = mode;
	props.startPosition = startPosition;
	props.labels = {};
	props.loop = loop;
	props.reversed = reversed;
	cjs.MovieClip.apply(this,[props]);

	this.actionFrames = [0,538];
	// timeline functions:
	this.frame_0 = function() {
		//if(!this.alreadyyExecuted){
		//this.alreadyyExecuted=true;
		//	this.clickthru_btn.on("click", function(evt){
		//  window.open(clickTag, "_blank");
		//});
		//} else {
		//gotoAndPlay(2);
		//}
	}
	this.frame_538 = function() {
		//if(!this.alreadyExecuted){
		//this.alreadyExecuted=true;
		//this.loopNum=1;
		//} else {
		//this.loopNum++;
		//if(this.loopNum==2){
		this.stop();
		//}
		//}
	}

	// actions tween:
	this.timeline.addTween(cjs.Tween.get(this).call(this.frame_0).wait(538).call(this.frame_538).wait(1));

	// border
	this.shape = new cjs.Shape();
	this.shape.graphics.f().s("#999999").ss(1,2,0,3).p("Eg4ygG8MBxlAAAIAAN5MhxlAAAg");
	this.shape.setTransform(364,45);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(539));

	// PIC1
	this.instance = new lib.PIC1();
	this.instance.setTransform(369.95,39.1,0.735,0.735,0,0,0,0.1,59.1);

	this.timeline.addTween(cjs.Tween.get(this.instance).wait(143).to({alpha:0},7).to({_off:true},1).wait(388));

	// HEAD1
	this.instance_1 = new lib.HEAD1();
	this.instance_1.setTransform(126,72,1,1,0,0,0,0,24);
	this.instance_1.alpha = 0;

	this.timeline.addTween(cjs.Tween.get(this.instance_1).to({y:32,alpha:1},13,cjs.Ease.quadOut).wait(130).to({alpha:0},7).to({_off:true},1).wait(388));

	// PIC2
	this.instance_2 = new lib.PIC2();
	this.instance_2.setTransform(369.95,39.1,0.735,0.735,0,0,0,0.1,59.1);
	this.instance_2._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_2).wait(143).to({_off:false},0).wait(118).to({alpha:0},7).to({_off:true},1).wait(270));

	// HEAD2
	this.instance_3 = new lib.HEAD2();
	this.instance_3.setTransform(126,72,1,1,0,0,0,0,24);
	this.instance_3.alpha = 0;
	this.instance_3._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_3).wait(151).to({_off:false},0).to({y:32,alpha:1},13,cjs.Ease.quadOut).wait(97).to({alpha:0},7).to({_off:true},1).wait(270));

	// PIC3
	this.instance_4 = new lib.PIC3();
	this.instance_4.setTransform(369.95,39.1,0.735,0.735,0,0,0,0.1,59.1);
	this.instance_4._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_4).wait(261).to({_off:false},0).wait(125).to({alpha:0},7).to({_off:true},1).wait(145));

	// HEAD3
	this.instance_5 = new lib.HEAD3();
	this.instance_5.setTransform(126,72,1,1,0,0,0,0,24);
	this.instance_5.alpha = 0;
	this.instance_5._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_5).wait(269).to({_off:false},0).to({y:32,alpha:1},13,cjs.Ease.quadOut).wait(104).to({alpha:0},7).to({_off:true},1).wait(145));

	// PIC4
	this.instance_6 = new lib.PIC4();
	this.instance_6.setTransform(369.95,39.1,0.735,0.735,0,0,0,0.1,59.1);
	this.instance_6._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_6).wait(386).to({_off:false},0).wait(139).to({alpha:0},7).to({_off:true},1).wait(6));

	// HEAD4
	this.instance_7 = new lib.HEAD4();
	this.instance_7.setTransform(174,72,1,1,0,0,0,0,24);
	this.instance_7.alpha = 0;
	this.instance_7._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_7).wait(394).to({_off:false},0).to({y:32,alpha:1},13,cjs.Ease.quadOut).wait(118).to({alpha:0},7).to({_off:true},1).wait(6));

	// END SCREEN
	this.instance_8 = new lib.EndScreen();
	this.instance_8.setTransform(369.95,39.1,0.735,0.735,0,0,0,0.1,59.1);
	this.instance_8.alpha = 0;
	this.instance_8._off = true;

	this.timeline.addTween(cjs.Tween.get(this.instance_8).wait(525).to({_off:false},0).to({alpha:1},13,cjs.Ease.quadOut).wait(1));

	// LOGO
	this.instance_9 = new lib.LOGO();
	this.instance_9.setTransform(615.75,48.9,1,1,0,0,0,7.9,21.7);

	this.timeline.addTween(cjs.Tween.get(this.instance_9).wait(539));

	// BKGD-rubine
	this.instance_10 = new lib.BKGDrubine();
	this.instance_10.setTransform(364.25,69.2,2.4267,0.36,0,0,0,0.1,67.2);

	this.timeline.addTween(cjs.Tween.get(this.instance_10).wait(539));

	this._renderFirstFrame();

}).prototype = p = new lib.AnMovieClip();
p.nominalBounds = new cjs.Rectangle(363.5,40.7,365,86.3);
// library properties:
lib.properties = {
	id: '0957C5EC3B874C629166840920B09671',
	width: 728,
	height: 90,
	fps: 36,
	color: "#999999",
	opacity: 1.00,
	manifest: [
		{src:"images/CO_AEP_728x90_atlas_NP_1.jpg?1666217878564", id:"CO_AEP_728x90_atlas_NP_1"}
	],
	preloads: []
};



// bootstrap callback support:

(lib.Stage = function(canvas) {
	createjs.Stage.call(this, canvas);
}).prototype = p = new createjs.Stage();

p.setAutoPlay = function(autoPlay) {
	this.tickEnabled = autoPlay;
}
p.play = function() { this.tickEnabled = true; this.getChildAt(0).gotoAndPlay(this.getTimelinePosition()) }
p.stop = function(ms) { if(ms) this.seek(ms); this.tickEnabled = false; }
p.seek = function(ms) { this.tickEnabled = true; this.getChildAt(0).gotoAndStop(lib.properties.fps * ms / 1000); }
p.getDuration = function() { return this.getChildAt(0).totalFrames / lib.properties.fps * 1000; }

p.getTimelinePosition = function() { return this.getChildAt(0).currentFrame / lib.properties.fps * 1000; }

an.bootcompsLoaded = an.bootcompsLoaded || [];
if(!an.bootstrapListeners) {
	an.bootstrapListeners=[];
}

an.bootstrapCallback=function(fnCallback) {
	an.bootstrapListeners.push(fnCallback);
	if(an.bootcompsLoaded.length > 0) {
		for(var i=0; i<an.bootcompsLoaded.length; ++i) {
			fnCallback(an.bootcompsLoaded[i]);
		}
	}
};

an.compositions = an.compositions || {};
an.compositions['0957C5EC3B874C629166840920B09671'] = {
	getStage: function() { return exportRoot.stage; },
	getLibrary: function() { return lib; },
	getSpriteSheet: function() { return ss; },
	getImages: function() { return img; }
};

an.compositionLoaded = function(id) {
	an.bootcompsLoaded.push(id);
	for(var j=0; j<an.bootstrapListeners.length; j++) {
		an.bootstrapListeners[j](id);
	}
}

an.getComposition = function(id) {
	return an.compositions[id];
}


an.makeResponsive = function(isResp, respDim, isScale, scaleType, domContainers) {		
	var lastW, lastH, lastS=1;		
	window.addEventListener('resize', resizeCanvas);		
	resizeCanvas();		
	function resizeCanvas() {			
		var w = lib.properties.width, h = lib.properties.height;			
		var iw = window.innerWidth, ih=window.innerHeight;			
		var pRatio = window.devicePixelRatio || 1, xRatio=iw/w, yRatio=ih/h, sRatio=1;			
		if(isResp) {                
			if((respDim=='width'&&lastW==iw) || (respDim=='height'&&lastH==ih)) {                    
				sRatio = lastS;                
			}				
			else if(!isScale) {					
				if(iw<w || ih<h)						
					sRatio = Math.min(xRatio, yRatio);				
			}				
			else if(scaleType==1) {					
				sRatio = Math.min(xRatio, yRatio);				
			}				
			else if(scaleType==2) {					
				sRatio = Math.max(xRatio, yRatio);				
			}			
		}
		domContainers[0].width = w * pRatio * sRatio;			
		domContainers[0].height = h * pRatio * sRatio;
		domContainers.forEach(function(container) {				
			container.style.width = w * sRatio + 'px';				
			container.style.height = h * sRatio + 'px';			
		});
		stage.scaleX = pRatio*sRatio;			
		stage.scaleY = pRatio*sRatio;
		lastW = iw; lastH = ih; lastS = sRatio;            
		stage.tickOnUpdate = false;            
		stage.update();            
		stage.tickOnUpdate = true;		
	}
}
an.handleSoundStreamOnTick = function(event) {
	if(!event.paused){
		var stageChild = stage.getChildAt(0);
		if(!stageChild.paused || stageChild.ignorePause){
			stageChild.syncStreamSounds();
		}
	}
}
an.handleFilterCache = function(event) {
	if(!event.paused){
		var target = event.target;
		if(target){
			if(target.filterCacheList){
				for(var index = 0; index < target.filterCacheList.length ; index++){
					var cacheInst = target.filterCacheList[index];
					if((cacheInst.startFrame <= target.currentFrame) && (target.currentFrame <= cacheInst.endFrame)){
						cacheInst.instance.cache(cacheInst.x, cacheInst.y, cacheInst.w, cacheInst.h);
					}
				}
			}
		}
	}
}


})(createjs = createjs||{}, AdobeAn = AdobeAn||{});
var createjs, AdobeAn;